...
`routing.Logger` injected (not `nil` or a no-op sink). When `RouteFrame` encounters an
HMAC verification failure, the log event E-ADM-016 is written to the logger's output.
...
On each tick, if `accessNode.FramesDropped()` > 0, a structured log line is written at
INFO level: `"frames_dropped count=<N>"` (or equivalent structured key-value format).
...
| EC-003 | ADR-011 §Concurrency | `sc.frames` full when relay tries to write a new frame | Frame dropped (non-blocking select); `FramesDropped` counter incremented; no panic |
...
8. [ ] Verify Red Gate: all 6 daemon tests fail before access.go implementation
9. [ ] Implement access.go (wiring body) to make tests green
10. [ ] Verify ARCH-08 §6.5.2: `cmd/switchboard` imports only
        `{internal/admission, internal/routing, internal/session, internal/tmux, internal/halfchannel}`
        plus stdlib — no `internal/config`, `internal/drain`, `internal/metrics`
...
|------|--------|-------------|
| `cmd/switchboard` imports only positions 1–7 plus stdlib | ARCH-08 §6.5.2 (import set enumerated) | `go vet` import cycle detection; lint |
...
don't exist on develop and cannot be imported; CI enforces this structurally).
...
| `cmd/switchboard/access.go` | create | Access-mode wiring: all six obligations from ARCH-08 §6.5.1 |
...
| `cmd/switchboard/main_test.go` | create / extend | Integration tests: AC-001 through AC-008 |
