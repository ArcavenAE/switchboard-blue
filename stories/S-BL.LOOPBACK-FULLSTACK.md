...
inputDocuments:
  ...
  - '.factory/stories/S-BL.PE-RECEIVE-LOOP.md'   # precedent for the Env.wg/closeCh ticker-goroutine idiom (Q6) and story-writer conventions (grep-resolved symbols; no self-referential story-line-number citations — disk-verified external-source anchors permitted)
...
(`.factory/decisions/S-BL.LOOPBACK-FULLSTACK-placement-note.md` v1.13) — **story-writer's job here is
...
**[v1.4 B-2] Window-safety invariant for CreateSession-time start:** `chanSeq` at the
...
(binding per placement note v1.13):** AC-013 verifies via the direct invocation the merged file's own doc
...
**Second forward obligation (placement note v1.13 Risks item 6):** this story's binding `RoundTrip`-token
