...
behavioral_contracts:
  ...
  - BC-2.01.003   # independent upstream/downstream half-channel clocks + sequence spaces (harness: independent tick intervals per AC-002; replay precondition out of scope per Q1)
  - BC-2.02.001   # duplicate-and-race dispatch
  - BC-2.02.002   # endpoint checksum-only dedup
  - BC-2.02.003   # per-path RTT/loss ranked via keep-alive probes (harness: paths.Rank() dispatch exercised by-construction per AC-004; probe-driven RTT/loss tracking out of scope, PARTIAL/HARNESS-SCOPE)
...
vp_traces:
  - VP-042   # keystroke-to-echo p99 <= 100ms — harness delivery only; lock flip is a separate subsequent act, see Non-Goals / Forward Obligation
subsystems: [session-networking, multipath-forwarding]
...
(`.factory/decisions/S-BL.LOOPBACK-FULLSTACK-placement-note.md` v1.16) — **story-writer's job here is
...
| Independent upstream/downstream half-channel clocks and sequence spaces | BC-2.01.003 | placement note Q1, Q6 | TO DISCHARGE (partial, harness-scope) — the upstream and downstream `HalfChannel`s run on independently-configured tick intervals (`cfg.TickIntervalUpstream`/`cfg.TickIntervalDownstream`) with separate per-direction sequence spaces; both ticker goroutines fire `HalfChannel.Tick()` on their own schedule independent of `Enqueue` timing (AC-002). BC-2.01.003's full contract is NOT met — its Precondition 3 (upstream recovery is idempotent replay, CAP-007) is out of scope per Q1/ARCH-03 (this harness wires no upstream replay); only the independent-clock/sequence-space aspect is anchored: clock/interval independence is exercised by AC-002(b)'s `TestLoopbackDriver_TicksFireOnSchedule`; sequence-space independence holds by construction (two separate `halfchannel.New()` instances, each with its own private `seq uint32`; `multipath.Frame` carries no cross-channel sequence field) and is not itself test-asserted. |
...
| Per-path RTT and loss tracked via keep-alive probes; paths ranked by quality | BC-2.02.003 | placement note Q7 | TO DISCHARGE (partial, harness-scope) — `multipath.Send` calls `paths.Rank()` on every send (`multipath.go:252`), so both `paths.RankedPath`s per direction are ranked each dispatch (AC-004) — incidental to the BC-2.02.001 duplicate-and-race assertion, not itself asserted. BC-2.02.003's actual subject — keep-alive-probe-driven per-path RTT/loss tracking and quality adaptation — is NOT exercised: `paths.NewPathTracker(1.0, 0.125)` trackers are static, `IsActive()` holds by construction (`paths.go:115`), zero `OnProbe` calls, no simulated probe/loss (consistent with Non-Goals) |
| Downstream ARQ (piggybacked ACK/SACK) | BC-2.02.005 | placement note Q1, Q4 + Q4 Addendum | TO DISCHARGE (downstream leg only — upstream ARQ is explicitly out of scope per Q1/ARCH-03) — every downstream tick's data frame passes through `driver.downstreamARQ.EnqueueSend`; the SAME tick body then calls the SAME `driver.downstreamARQ.OnAck` exactly once, in the tick body after `Send`/`deliverDownstream` return, per the Q4 call-contract (single shared instance — AC-001 **DISCHARGED**, verdict REVISED) |
...
triggers an out-of-band delivery. (BC-2.01.001 compliance; the independently-configured per-direction
schedule this test exercises is also the BC-2.01.003 harness-scope clock-independence aspect. Sequence-space
independence — the second BC-2.01.003 harness-scope aspect — holds by construction (each direction owns its
own `halfchannel.New()` instance and a private `seq uint32`; `multipath.Frame` carries no cross-channel
sequence field), not by test assertion; see Anchors Consumed.)
...
once per selected path. (BC-2.02.001 compliance; the two-path `multipath.Send` dispatch this test
exercises is also the BC-2.02.003 harness-scope path-ranking aspect — `multipath.Send` calls
`paths.Rank()` on every dispatch (`multipath.go:252`), incidental to this AC's duplicate-and-race
assertion, not itself asserted. BC-2.02.003's probe-driven per-path RTT/loss tracking and quality
adaptation is NOT exercised — see Anchors Consumed.)
...
(binding per placement note v1.16):** AC-013 verifies via the direct invocation the merged file's own doc
...
**Second forward obligation (placement note v1.16 Risks item 6):** this story's binding `RoundTrip`-token
...
| Version | Date | Change |
|---------|------|--------|
| 1.21 | 2026-08-30 | Human-gate-directed spec edit (§1.7 audit finding F-S1.7-R37-01, QUESTION-severity, human-directed), story-frontmatter+body-only, placement note unchanged at v1.16. BC-2.02.003 ("Per-Path RTT and Loss Tracked via Keep-Alive Probes; Paths Ranked by Quality") anchored PARTIAL/HARNESS-SCOPE, exercised-but-previously-unanchored per the §1.7 audit finding: added to `behavioral_contracts:`/`bc_traces:` frontmatter (numeric order, between BC-2.02.002 and BC-2.02.005), a new Anchors Consumed table row (same position, citing placement note Q7 — the synthetic path/paths-machinery design source), and an AC-004 trace annotation (`### AC-004 (traces to BC-2.02.001, BC-2.02.003; Q3, Q7)` + a parenthetical on the body paragraph) — `multipath.Send` calls `paths.Rank()` on every send (`multipath.go:252`), so both `paths.RankedPath`s per direction are ranked each dispatch, exercised incidentally by AC-004's duplicate-and-race assertion (not itself asserted as a BC-2.02.003 outcome). BC-2.02.003's actual subject — keep-alive-probe-driven per-path RTT/loss tracking and quality adaptation — is NOT exercised: `paths.NewPathTracker(1.0, 0.125)` trackers are static, `IsActive()` holds by construction (`paths.go:115`), zero `OnProbe` calls, no simulated probe/loss (consistent with Non-Goals) — so the anchor is PARTIAL/HARNESS-SCOPE, mirroring the v1.19 BC-2.01.003 partial-anchor treatment (GAP-4). No new AC (AC count 17 unchanged — existing AC-004 gains a second BC trace). Points 8 unchanged; status draft unchanged; subsystems `[session-networking, multipath-forwarding]` unchanged. Input-hash UNCHANGED at `7967a2f` (BC anchoring and comment text are not declared inputs — the declared `inputs:`/note/VP-042/ARCH-08/ARCH-03 content is unchanged; confirmed via rc.24 `compute-input-hash --update`, no drift). Step-4.5 adversarial convergence counter RESET by this spec edit (expected, per the R14/v1.19 precedent — a converged spec that is then edited must reconverge). |
| 1.20 | 2026-08-29 | R33 adversarial reconvergence repair (POL-005 dispatch), fixing two architect-ruled-FIX LOW findings, story-frontmatter+body-only. Finding-1 — the placement note advanced v1.15→v1.16, dropping a stale "TLPKTDROP" mislabel from an L15 comment in the note's own text (cosmetic, no binding design change); because the note is this story's declared input (`inputs:` L19), the story's `input-hash` is recomputed `4902d5d`→`7967a2f` via rc.24 `compute-input-hash --update`, confirmed. Every live current-version note reference in this story's body advanced v1.15→v1.16: the `inputDocuments` L59 leading pin (`# v1.15 — BINDING` → `# v1.16 — BINDING`, plus this v1.20 entry appended), the Context paragraph's binding-input citation, the AC-013 "Verification Method (binding per placement note vX)" header, the File Structure Requirements Forward Obligation row's "Per placement note vX Risks item 6" citation, and the Forward Obligation section's "Second forward obligation (placement note vX Risks item 6)" citation; the top-of-file status-note blockquote (L74) gains a v1.20 entry. Finding-2 (F-R33-LENSB-02, LOW) — AC-002(b) and the BC-2.01.003 Anchors Consumed row overclaimed that AC-002's tests "exercise" BC-2.01.003's sequence-space-independence aspect; AC-002(b)'s parenthetical and the Anchors Consumed row's trailing clause are both tightened to state precisely that only clock/interval independence is test-exercised (`TestLoopbackDriver_TicksFireOnSchedule`) — sequence-space independence is a by-construction property (each direction owns its own `halfchannel.New()` instance and a private `seq uint32`; `multipath.Frame` carries no cross-channel sequence field), not itself test-asserted. AC count 17 unchanged (no new AC; existing AC-002(b) and the BC-2.01.003 Anchors row wording tightened in place — no new BC trace). Points 8 unchanged; status draft unchanged; subsystems `[session-networking, multipath-forwarding]` unchanged. Step-4.5 adversarial convergence counter stays RESET at 0/3 (unchanged by this v1.20 pass, per the v1.19 reset). |
| 1.19 | 2026-08-29 | Human-gate-directed spec edit (§1.7 audit-2 GAP-4 + GAP-1), story-frontmatter+body-only, placement note unchanged at v1.15. GAP-4 — BC-2.01.003 ("Upstream and Downstream Half-Channels Operate with Independent Clocks and Sequence Spaces") anchored PARTIAL/HARNESS-SCOPE, exercised-but-previously-unanchored per the §1.7 audit-2 finding: added to `behavioral_contracts:`/`bc_traces:` frontmatter (numeric order, between BC-2.01.002 and BC-2.02.001), a new Anchors Consumed table row (same position), and an AC-002 trace annotation (`### AC-002 (traces to BC-2.01.001, BC-2.01.003; Q6)` + a parenthetical on sub-point (b)) — the upstream/downstream `HalfChannel`s run on independently-configured tick intervals (`cfg.TickIntervalUpstream`/`cfg.TickIntervalDownstream`) with separate per-direction sequence spaces, exercised by AC-002; BC-2.01.003's Precondition 3 (upstream recovery is idempotent replay, CAP-007) is NOT met — out of scope per Q1/ARCH-03 (this harness wires no upstream replay) — so the anchor is PARTIAL/HARNESS-SCOPE, mirroring the existing BC-2.01.002 partial-anchor treatment. GAP-1 (LOW, cosmetic) — the stale "TLPKTDROP" mislabel removed from three spots (the `behavioral_contracts:`/`bc_traces:` BC-2.02.005 comments, and the Anchors Consumed table's BC-2.02.005 row header): TLPKTDROP is BC-2.02.006's subject, a separate BC deliberately excluded per Non-Goals, not BC-2.02.005's. No new AC (AC count 17 unchanged — existing AC-002 gains a second BC trace). Points 8 unchanged; status draft unchanged; subsystems `[session-networking, multipath-forwarding]` unchanged. Input-hash UNCHANGED at `4902d5d` (BC anchoring and comment text are not declared inputs — the declared `inputs:`/note/VP-042/ARCH-08/ARCH-03 content is unchanged; confirmed via read-only rc.24 `compute-input-hash`, no `--update` run). Step-4.5 adversarial convergence counter RESET by this spec edit (expected, per the R14 precedent — a converged spec that is then edited must reconverge). |
| 1.18 | 2026-08-29 | §1.7 fresh-context consistency audit (post-3/3, R24) remediation (POL-005 dispatch, `.factory/cycles/cycle-1/S-BL.LOOPBACK-FULLSTACK/consistency-audit-R24-3of3-2026-08-29.md`), fixing one HIGH finding and disk-verifying (with no story edit required) one LOW finding. HIGH (Finding 1) — `subsystems:` frontmatter anchor corrected for ARCH-INDEX Subsystem Registry conformance (`.factory/specs/architecture/ARCH-INDEX.md` L86-88, the MUST-use-exact-Name rule): `transport-layer` removed (not a registered subsystem Name — no such row exists in the registry); `multipath-forwarding` (SS-02, `internal/multipath`/`internal/arq`/`internal/paths`) added — the story anchors 3 BC-2.02.* contracts and 3 of its 5 `architecture_modules:` (`internal/arq`, `internal/multipath`, `internal/paths`) are SS-02-owned per the registry, yet SS-02 was omitted; `quality-observability` (SS-06, `internal/metrics`/`internal/mgmt`) dropped — the story touches no SS-06-owned module (no metrics/mgmt) and anchors no BC-2.06.*, its `internal/paths` use being the SS-02 forwarding sense, not the SS-06 shared sense; `session-networking` (SS-01, `internal/halfchannel`, BC-2.01.001/002) kept unchanged. Net: `subsystems: [transport-layer, quality-observability, session-networking]` → `subsystems: [session-networking, multipath-forwarding]`. `architecture_modules:` untouched (its lone unregistered entry, `internal/testenv`, is legitimate unregistered test-infra, a separate systemic registry gap out of this story's remediation scope). LOW (Finding 3) — the audit's stale `cmd/switchboard/access.go:460`/`:500` analogy-citation claim was disk-verified against this story's own live text (whole-file grep for every `access.go` mention) and found NOT APPLICABLE to the story body: both live story citations (Story-Sizing Rationale, and the §Q6 ticker-lifecycle paragraph) are already unanchored symbol-name form — "`cmd/switchboard/access.go`'s `startSweepTicker`/`startFramesDroppedTicker`" — carrying no line number to drift; disk-verified current lines (`cmd/switchboard/access.go`: call sites `startSweepTicker` L446, `startFramesDroppedTicker` L454; definitions `func startSweepTicker` L595, `func startFramesDroppedTicker` L635) confirm the underlying analogy claim remains true. The drifted `:460`/`:500` citations the audit's Spec-vs-Implementation-Drift table found live only in the placement note (`S-BL.LOOPBACK-FULLSTACK-placement-note.md`, e.g. its L497/L1976/L2423/L2473), which is out of scope for this story-only remediation burst — no story edit made or needed for Finding 3. Finding 2 (MEDIUM, VP-042.md's Source Contract narrower than this story's 5-BC anchor set) is DEFERRED, unchanged, to the already-declared Forward Obligation to update VP-042.md's Proof Harness Skeleton (placement note Risks item 6) — no story edit. `inputDocuments`/status-note pins NOT bumped — L57/L72 each gain a v1.18 entry documenting this repair, but the declared input `S-BL.LOOPBACK-FULLSTACK-placement-note.md` is unchanged at v1.15; this is a story-frontmatter+ledger-only repair. AC count 17 unchanged (no new AC; the subsystems fix is a frontmatter/registry-conformance correction, not a behavioral change). Points 8 unchanged. Step-4.5 adversarial convergence counter RESET by this spec edit (expected and correct per the R14 precedent — a converged spec that is then edited must reconverge). Input-hash UNCHANGED at `4902d5d` (neither `subsystems:` nor the access.go citation is a declared input; confirmed via read-only rc.24 `compute-input-hash`, no `--update` run). |
