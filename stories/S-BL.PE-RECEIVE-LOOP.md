...
- `internal/upstreamdial/connector_test.go` — 7 new unit tests (3 original + `TestConnector_ReceiveLoop_FlapCycleJoin_NoLeak` flap-cycle F-SP3-002 + `TestConnector_ReceiveLoop_ExitsOnReadError` read-error exit pin test F-SP5-001 [recipe corrected v1.11 F-SP11-001] + `TestConnector_ReceiveLoop_ExitsOnVersionMismatch` version-rejection pin F-SP11-001 + `TestConnector_ReceiveLoop_CtlFrameForwardedToCallback` forwarding-completeness pin F-SP17-001)
...
**Estimated new test count (forecast):** ~12 net-new (1 `frame_test` + 7 `connector_test` + 4
integration). **connector_test.go** gains 7 tests (3 original + 1 AC-005 flap-cycle re-homed
from integration per F-SP3-002 + 1 read-error exit pin test per F-SP5-001 [recipe corrected v1.11 F-SP11-001] + 1 version-mismatch companion pin per F-SP11-001 + 1 forwarding-completeness pin per F-SP17-001). **router_pe_receive_test.go** has 4 integration tests
