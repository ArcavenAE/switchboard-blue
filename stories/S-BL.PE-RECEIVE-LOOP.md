...
- `internal/upstreamdial/connector_test.go` — 9 new unit tests (3 original + `TestConnector_ReceiveLoop_FlapCycleJoin_NoLeak` flap-cycle F-SP3-002 + `TestConnector_ReceiveLoop_ExitsOnReadError` read-error exit pin test F-SP5-001 [recipe corrected v1.11 F-SP11-001] + `TestConnector_ReceiveLoop_ExitsOnVersionMismatch` version-rejection pin F-SP11-001 + `TestConnector_ReceiveLoop_CtlFrameForwardedToCallback` forwarding-completeness pin F-SP17-001 + `TestUpstreamdialImportPerimeter` import-perimeter regression guard F-IP1-001 + `TestConnector_BootstrapFrameTypePEConnect` outgoing bootstrap frame_type pin F-IP4-001)
...
| `TestConnector_BootstrapFrameTypePEConnect` | **[v1.25 — F-IP4-001 pin]** dialLoop bootstrap ChannelFrame flip observable on the wire — connector's FIRST outbound frame parses to FrameType==FrameTypePEConnect (0x06); kills silent revert of FO-PE-LOOP-001 to FrameTypeData (AC-003 PC-3) |
...
**Estimated new test count (forecast):** ~14 net-new (1 `frame_test` + 9 `connector_test` + 4
integration). **connector_test.go** gains 9 tests (3 original + 1 AC-005 flap-cycle re-homed
from integration per F-SP3-002 + 1 read-error exit pin test per F-SP5-001 [recipe corrected v1.11 F-SP11-001] + 1 version-mismatch companion pin per F-SP11-001 + 1 forwarding-completeness pin per F-SP17-001 + 1 import-perimeter regression guard per F-IP1-001 + 1 bootstrap frame_type pin per F-IP4-001). **router_pe_receive_test.go** has 4 integration tests
...
15. [x] Verify `go test -race -count=1 ./...` full green; `golangci-lint` 0 issues; `gofumpt` no diffs
16. [x] Verify `TestScanForLine_DetectsEFWD001ProductionEmission` still passes unmodified
*(Tasks 1-16 marked complete per note v1.22 checkbox ruling — delivered at commits c316aed stubs / a3d5117 RED / e85c9df+8e8296c+5274cf1 GREEN / 9792605 spec-side; verified by adversarial passes 1-4.)*
...
21. [x] F-IP4-001: `TestConnector_BootstrapFrameTypePEConnect` outgoing-bootstrap frame_type pin (per note v1.22 recipe; test-writer)
...
| Version | Date | Change |
|---------|------|--------|
| 1.25 | 2026-07-11 | Pass-4 adversarial adjudication F-IP4-001 propagation (POL-001). Frontmatter version 1.24 → 1.25. inputDocuments placement-note leading label v1.21 → v1.22; descriptor appended: pass-4 F-IP4-001 — outgoing bootstrap frame_type unpinned (revert-survives-suite hole, same class as F-SP17/F-SP18 receive-side); TestConnector_BootstrapFrameTypePEConnect added (connector 8→9, total ~14); Tasks 1-16 checkbox hygiene fixed. FCL row 5 Change cell: v1.25 F-IP4-001 annotation appended (TestConnector_BootstrapFrameTypePEConnect — accept-and-read fixture, ReadFull 44 + ParseOuterHeader + FrameType assertion); anchor cell: F-IP4-001 appended. Estimated Test Surface: new TestConnector_BootstrapFrameTypePEConnect row added to connector_test.go table; connector tests 8→9, total ~13→~14. File Structure Requirements connector_test.go: 8→9 tests, TestConnector_BootstrapFrameTypePEConnect added. Task 11 amended with v1.22 F-IP4-001 annotation. Task 21 added (marked complete): F-IP4-001 outgoing bootstrap frame_type pin. Tasks 1-16 marked [x] per checkbox ruling (verified complete at c316aed/a3d5117/e85c9df+8e8296c+5274cf1/9792605). |
