...
18. [ ] Implement `nodeIdentifyHandshake(conn net.Conn, r *routing.Router, routerPrivKey ed25519.PrivateKey, ks *admission.AdmittedKeySet, h netingress.NodeHandle) (svtnID [16]byte, nodeAddr [8]byte, err error)` in `cmd/switchboard/node_identify_wire.go`: `conn.SetDeadline(time.Now().Add(nodeIdentifyHandshakeTimeout))` on entry; `io.ReadFull` outer header + payload for NodeIdentify; validate SVTNID non-zero; decode NodeIdentify; derive `nodeAddr = frame.DeriveNodeAddress(hdr.SVTNID, pubkey)`; call `GenerateChallenge(routerPrivKey)`; encode + write Challenge; `io.ReadFull` outer header + payload for ChallengeResponse; decode ChallengeResponse; call `AdmitNode`; on success call `r.BindInterface(hdr.SVTNID, nodeAddr, h.IfaceID)` then `conn.SetDeadline(time.Time{})`; on any error path close conn and return error — implementer [BC-2.01.009 PC-1 through PC-8; §7 of rulings; failure table §13]
19. [ ] In the PER-CONN `route` closure (see Task 20), add `case 0x04:` (duplicate NodeIdentify on an established connection): log WARN `"node_identify: duplicate NodeIdentify on established connection (E-ADM-023)"`, then call `_ = conn.Close()` directly on the captured per-conn `net.Conn` — this makes `ServeConn`'s next `ReadFrame` return a net error, terminating `ServeConn` and triggering its deferred cleanup. Also `return errors.New("node_identify: E-ADM-023 duplicate NodeIdentify — connection closed")` as a belt-and-suspenders signal (ServeConn will drop it per the drop-and-continue contract, but the conn is already closed so the read loop terminates on the next read). Do NOT rely on the returned error for teardown — `netingress.ServeConn` intentionally drops route errors (drop-and-continue, BC-2.05.008); the close is what tears down the connection — implementer [BC-2.01.009 Invariant 7 + mechanism note (v1.2); §17 of rulings; AC-011]
...
20a. [ ] E-ADM-023 teardown wiring (§17 Option B): add an optional field `PerConnRoute func(conn net.Conn, h NodeHandle) RouteFn` to `netingress.ServeConfig` (additive, nil-safe — when nil, `Serve` uses the shared `route`, so all existing callers see zero behavior change and `ServeConfig{}` callers still allocate nothing). In `netingress.Serve`, after the `OnAccept` call, if `cfg.PerConnRoute != nil` call it to obtain this connection's route function and pass THAT to `ServeConn` instead of the shared `route`; otherwise pass the shared `route` unchanged. In `runRouter` (`cmd/switchboard/mgmt_wire.go`), set `ServeConfig.PerConnRoute` to a func that returns a per-conn route closure capturing this connection's `net.Conn`; that closure is identical to the shared `route` EXCEPT `case 0x04` calls `conn.Close()` (Task 19). CONSTRAINT: preserve the documented zero-allocation invariant (`netingress.go:214-216`) — the `NodeHandle`/Send/Done allocation must remain guarded so that a caller passing `ServeConfig{}` (both OnAccept and PerConnRoute nil) allocates nothing — implementer [BC-2.01.009 Invariant 7 mechanism note; §17 of rulings; AC-011]
...
| 1.9 | 2026-07-19 | §17 (E-ADM-023 teardown, Option B) cascade: Task 19 corrected — teardown is conn.Close() in the per-conn route closure, NOT the (dropped) route-error return; Task 20a added — PerConnRoute additive field on netingress.ServeConfig with zero-alloc-invariant constraint. ConnHandle→NodeHandle story/code drift fixed (Task 18). BC-2.01.009 re-pinned v1.1→v1.2; Invariant 7 mechanism note added. input-hash recomputed f32d7ee→e252875. |
