...
F-DWSP4-001 fix — not regressions. VP-080 v1.6 Test Scenario 5 exercises case (2); case (3) is not
yet test-scripted.
...
5. `uint64` composite wraparound behavior is explicitly OUT OF SCOPE — only relative ordering (`>`)
   is asserted; no wraparound case is tested (VP-080 v1.6 property 4, Non-Goals). The composite's
...
6. **(F-DWSP4-001, v1.5, new)** A restarted access node's first post-restart datagram — declaring a
   ...
   prior `lastSeen` watermark with overwhelming likelihood (VP-080 v1.6 Property 5). See EC-010 for
...
- **`uint64` composite `Sequence` wraparound handling** — out of scope for this story and VP-080
  v1.6 property 4 (see AC-010 postcondition 5 for the composite's per-component wrap bounds); not a
