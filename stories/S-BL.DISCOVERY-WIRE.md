...
relay fan-out integration test (formalizes the gap disclosed by `TestRunRouter_RelayFanOut_EndToEnd`'s
own comment — unit coverage exists for each link but no single test drives advertisement → frame
arrival across a live TCP connection, deferred as too flaky/heavy for a per-story gate).
...
| `cmd/switchboard/discovery_listener_wire_test.go` | create | **NEW (Task 6d)** — four tests: (a) `TestRunRouter_JoinsDiscoveryGroups_ForAdmittedSVTNs` — daemon-level multicast-group-join oracle; this is the RED-gate test for Task 6d and the primary AC-001 daemon-lifecycle verifier; (b) `TestRunRouter_DiscoveryListeners_CleanShutdown` — `discoveryWG` teardown green-guard (verifies clean shutdown with admitted SVTNs after wiring); (c) `TestOnRelayClosureConcurrentAccess` — `relayRateCap` concurrent-access `-race` lock-in; (d) `TestRunRouter_RelayFanOut_EndToEnd` — **deferred/green-guard placeholder only**: the full runtime relay chain (`runRouter` → `RouterIngest` → `onRelay` closure → `relayRateCap.allow` → `relayDispatch`) is NOT exercised end-to-end; the test passes trivially pre-wiring and becomes a clean-shutdown discriminator post-wiring; per the file's own "JUDGMENT CALL — LIGHTER FLAG-AND-DEFER" and "FULL E2E RELAY COVERAGE GAP" comments (lines 166–202), the full e2e relay chain (send HMAC-authenticated multicast datagram → observe `DISCOVERY_RELAY` frame on second admitted node's TCP connection) is deferred to a follow-on integration test; the relay chain is unit-covered by `TestRelayDispatch_*` (Task 6b, `discovery_relay_wire_test.go`) and `TestRelayDispatch_RateCap_*` (Task 6c, same file) plus code-inspection of the wiring. |
