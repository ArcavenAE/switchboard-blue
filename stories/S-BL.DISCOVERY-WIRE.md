...
modified:
  ...
  - date: 2026-07-20
    ...
    change: >
      ...
      `relay_rate_cap_test.go` corrected modify→create (file absent at develop, added 5d7cdb1,
      contains only the 3 map-bounding tests; sibling of the v2.22
      discovery_wire_map_bounding_test.go correction). (F-2 MED) top-of-body Status note
      corrected — claimed draft/not-promoted, contradicting frontmatter status: ready (v2.12
      promoted 2026-07-14). (F-3 LOW) Provenance Status bullet explicitly scoped as v2.0-era
      historical snapshot. Map-bounding-ruling v1.1→v1.2 citation sweep (Pass-2 F-1: ruling
      Decision 8 self-eviction guarantee corrected to improbable-not-impossible; benign if
      evicted). input-hash 7ff0732→def6b7b (ruling input bytes changed). count 18, points 8,
      status ready — unchanged.
  - date: 2026-07-20
    version: "2.23"
    change: >
      Pass-C F-1 fix — corrected 2 File-Change-List rows citing map-bounding-ruling.md v1.0 →
      v1.1 (Decisions 1 & 4 are byte-identical v1.0→v1.1; content was accurate, version pin
      stale). Body-only; input-hash unchanged at 7ff0732 (no inputs: file changed). count stays
      18, points stay 8, status stays ready.
  - date: 2026-07-20
    version: "2.22"
    change: >
      Map-bounding ruling v1.0→v1.1 adjudication (Decision 8 / Option A): both-paths bound
      blessed, delivered code (545429f) stands as-is. FCL test-file-path correction:
      internal/discovery/discovery_wire_map_bounding_test.go (create) replaces the v2.21
      stale row naming discovery_wire_test.go (modify). SEC-DW-10 clause updated to reflect
      v1.1 contract. inputDocuments ruling comment v1.0→v1.1. input-hash recomputed:
      b868e80 → 7ff0732. acceptance_criteria_count stays 18; points stay 8; status stays
      ready.
  - date: 2026-07-20
    version: "2.21"
    change: >
  ...
  - date: 2026-07-20
    ...
    change: >
      ...
      test names, or acceptance_criteria_count (stays 18) changed; points stay 8;
      status stays ready; input-hash stays 5a4d0da (body-only edits — no declared
      inputs: file changed). LOW-1: three live-body references to the stale test
      name TestRunRouter_RelayFanOut_EndToEnd (old RED-phase placeholder name)
      updated to delivered name TestRunRouter_WithAdmittedSVTN_ShutsDownCleanly in
      the FO(h) body, the FO summary paragraph, and the File-Change List row (d);
      v2.18 and v2.19 Changelog rows preserved as historical provenance with a
      parenthetical forward-pointer per this story's established
      historical-preservation convention. The test is a shutdown-teardown guard, NOT
      an e2e relay test — that gap is FO(h) itself. LOW-2: FO(g) prose sharpened to
      state the operational materiality plainly — in the standard cold-start
      deployment (router up → control pushes keys), AllSVTNEntries() startup snapshot
      is empty → ZERO discovery listeners spawned → hop-2 discovery-relay is inert
      until a router restart with a populated snapshot; FO(g) deferral status
      unchanged (non-blocking, task6d-wiring-seam-ruling.md v1.0 Decision 5).
  - date: 2026-07-20
    version: "2.19"
    change: >
      Step-4.5 pass-2 MED + LOW-2 process-half remediation. FIX 1 (MED, POL-002):
      STORY-INDEX.md row 144 col-3 was stale and self-contradictory — "AC-017/018/Task 6 gated
      on S-BL.NODE-IDENTIFY-WIRE" is FALSE (gate SATISFIED PR #127; Task 6a-6d CODE-COMPLETE
      v2.18); cited v2.16/v2.17 not v2.18; contradicted col-4's CODE-COMPLETE/all-6-gates-green
      claim. Updated to accurate current-state summary (STORY-INDEX.md edited in this same
      burst — note only, actual row edit is in that file). FIX 2 (LOW-2 process-half):
      Forward Obligations table gained row (h) — full-daemon end-to-end relay fan-out integration
      test. The runtime chain runRouter → wireDiscoveryListener → RouterIngest.Ingest → onRelay
      → relayRateCap.allow → relayDispatch → peer nc.send is unit-covered per link (TestRelayDispatch_*
      6b/6c, onRelay-seam test 6d, daemon-join oracle) but NOT by a single e2e integration test
      sending a real HMAC-authenticated advertisement and observing DISCOVERY_RELAY on a live TCP
      connection. Deferred as too flaky/heavy for a deterministic per-story gate; formalizes the
      gap TestRunRouter_RelayFanOut_EndToEnd's own comment discloses. FO summary paragraph
      updated: (h) newly opened + deferred alongside (g). Version 2.18 → 2.19. input-hash stays
      5a4d0da (body-only edits; no declared inputs: file changed). acceptance_criteria_count stays
      18; points stay 8; status stays ready.
  - date: 2026-07-20
    version: "2.18"
    change: >
      Step-4.5 pass-1 spec-drift reconciliation (MED-1 + MED-2). AC-001 Scope note
      superseded in place: reflects FO(e) RESOLVED (Task 6d, af91335); wireDiscoveryListener
      IS now called from runRouter; TestRunRouter_JoinsDiscoveryGroups_ForAdmittedSVTNs
      now verifies daemon-level join; residual FO(g). File-Change List row for
      discovery_listener_wire_test.go corrected: full runRouter->relayDispatch e2e relay chain
      NOT exercised end-to-end (FLAG-AND-DEFER per the file's own disclaimer); four delivered
      tests named accurately. No AC postconditions/test names changed; acceptance_criteria_count
      stays 18; points stay 8; status stays ready; input-hash stays 5a4d0da (body-only).
      Note: two concurrent HIGH code/test fixes (non-UTF-8 session name crash + onRelay
      integration test) are being applied in worktree af91335 separately from this spec burst.
  - date: 2026-07-20
    version: "2.17"
    change: >
      Task 6a-6d delivered — bookkeeping reconciliation (pre-Step-4.5 adversary). Applied
      decisions/S-BL.DISCOVERY-WIRE-task6d-wiring-seam-ruling.md v1.0 (five decisions: onRelay
      seam / backward-compat nil / rate-cap concurrency ownership / discoveryWG startup-loop
      lifecycle / FO(g) dynamic-SVTN deferral). inputs added task6d ruling; input-hash c8fe5f2
      -> 5a4d0da. FO(e) RESOLVED (wireDiscoveryListener startup loop wired, af91335). FO(g)
      added (dynamic post-startup SVTN join deferred). File-Change List updated to delivered 9-file
      delta. acceptance_criteria_count stays 18; points stay 8; status stays ready.
  - date: 2026-07-19
    version: "2.16"
    change: >
      AC-017/018 gate-clear sweep (v2.15 follow-on). Full-body scan for residual GATED/Human Gate
      item 3/Forward Obligation (a)/depends_on S-BL.NODE-IDENTIFY-WIRE-as-gate pins outside the
      Changelog. Ten live-body spots fixed; Changelog row added. No inputs changed; input-hash stays
      c8fe5f2. acceptance_criteria_count stays 18; points stay 8; status stays ready.
  - date: 2026-07-19
    version: "2.15"
    change: >
      Fan-out-resolution ruling applied — `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md`
      v1.0 (committed f7959c44, verified against develop @ `7fcf0cf`). Three coordinated changes:
      (1) **Task 6 decomposed** into 6a (`Router.InterfacesForSVTN(svtnID [16]byte, excludeNodeAddr
      [8]byte) []InterfaceID` in `internal/routing/identity.go`, unit tests in
      `internal/routing/identity_test.go`), 6b (relay-dispatch closure INLINE in `runRouter` NOT a
      package var, integration tests in `cmd/switchboard/discovery_relay_wire_test.go`), 6c (AC-018
      ~1/sec per-`(SVTNID,NodeAddr)` rate cap at dispatch-decision point, SEC-DW-09), 6d
      (`wireDiscoveryListener` startup loop wired into `runRouter` iterating
      `routerKS.AllSVTNEntries()` snapshot — FO(e) residual, now unblocked). (2) **AC-017/AC-018
      gate tags cleared**: `[GATED — depends_on S-BL.NODE-IDENTIFY-WIRE]` → `[UNGATED —
      S-BL.NODE-IDENTIFY-WIRE merged PR #127]`; Gate paragraphs updated to state dependency
      satisfied and target-resolution mechanism is `Router.InterfacesForSVTN` per ruling v1.0;
      AC postconditions and test names UNCHANGED. (3) **Forward Obligations (e)/(f) reconciled**:
      (e) INFRASTRUCTURE-RESOLVED — `wireAdmissionSyncHandlers` is now in `runRouter` (V9),
      `AllSVTNEntries()`/`ListBySVTN()` exist (V7/V8); only residual is the `wireDiscoveryListener`
      wiring call (Task 6d), unblocked; (f) CLOSED — S-BL.NODE-ADMISSION-PROVISIONING (PR #125 @
      ce06f6a) delivered `loadOrGenerateAdmissionKeypair` + `runAccessWithConnector` calling
      `d.Run(runCtx)` (V11). FO summary paragraph updated accordingly. Ruling added to `inputs:`
      and `inputDocuments:`. `input-hash` recomputed via `compute-input-hash --update`: c8aaf9c →
      c8fe5f2. `acceptance_criteria_count` stays 18; points stay 8. `status` stays `ready`.
  - date: 2026-07-15
    version: "2.14"
    change: >
      Pre-adjudication cascade from `S-BL.DISCOVERY-WIRE-rulings.md` v1.11 (Ruling 5 — the
      F-DWIP1-001 sender-side key-derivation fix sanctioned as Ruling-1-faithful, plus a new,
      independently-discovered obligation naming node-side admission-identity provisioning as a
      THIRD, distinct leg of the identity-distribution cluster). **AC-004 gained a SECOND
      qualifying note (F-DWIP1-001, v1.11), appended after the existing F-DWSP8-001 note:**
      `Encode`/`Decode`'s signatures gained an explicit `nodeAdmissionPubkey []byte` parameter
      (both route through `routing.DeriveDiscoveryKey`, confirmed faithful to postcondition 4's
      already-specified symmetric design) after a Step-4.5 pass-1 fix burst found the shipped
      `Encode`/`Decode` had regressed to deriving the discovery HMAC key from cleartext `SVTNID`
      alone — the exact anti-pattern DRIFT-W6TBD-001/Ruling 1 already rejected — breaking
      sender↔router interop undetected because no prior test exercised a real
      `Encode`→router-`Ingest` round-trip. New integration test:
      `TestDiscovery_EncodeThenRouterIngest_AcceptsRealAdmittedNode`. `discovery.Config` gained
      `LocalNodeAdmissionPubkey []byte`; `transmitAdvertisement` fails closed with
      `ErrMissingNodeAdmissionPubkey` when empty. Production usability of this fix is itself gated
      on a new Forward Obligation (row (f)). **Forward Obligations table gained row (f):**
      node-side admission-identity provisioning — no production code path anywhere supplies a
      running access-node process with its own admission keypair (`internal/config.Config` has no
      admission-keypair field; `runAccess` constructs only an ephemeral mgmt keypair and its own
      separate, empty `AdmittedKeySet`). Independently found to be compounded by a second, distinct
      gap: `internal/discovery.New`/`Discovery.Run` have zero production callers anywhere in the
      repository — the sender daemon-lifecycle wiring into `runAccess` was never built, independent
      of identity. Same root cause also blocks `S-BL.NODE-IDENTIFY-WIRE`'s own `ChallengeResponse`
      signing (needs the node's private key, which nothing provisions either). New follow-on story
      recommended: `S-BL.NODE-ADMISSION-PROVISIONING` (working name — not yet created; PO/architect
      to confirm name + scope), distinct from `S-BL.ADMISSION-SYNC-WIRE` (row (e)) — opposite
      direction (node learns its OWN identity, vs. router/console learning OTHER nodes' identities).
      `S-BL.NODE-IDENTIFY-WIRE` must add this story as a SECOND `depends_on` prerequisite alongside
      `S-BL.ADMISSION-SYNC-WIRE` once both exist. **Forward Obligations summary paragraph
      replaced:** obligations (a) and (d) resolved as of 2026-07-14; (b)/(c)/(e)/(f) remain open
      and non-blocking; none of (b)/(c)/(d)/(e)/(f) ever blocked TDD implementation of
      AC-001..AC-016 — (e)/(f) are retrospective, implementation-time findings (Ruling 4 v1.10,
      Ruling 5 v1.11) accepted at function level with qualifying notes on AC-001/AC-004, not
      `[GATED]` markers; AC-017/AC-018/Task 6 gate on `S-BL.NODE-IDENTIFY-WIRE` by name, Tasks 1-5
      remain independently deliverable regardless of that story's schedule. **AC-003: no change**
      — architect confirmed key derivation is AC-004 territory, the Ruling 2 addendum's multicast
      egress elaboration is untouched by Ruling 5. Mechanical version-pin sweep: every live-prose
      `rulings v1.10` pin → `v1.11` at 16 spots (the `inputDocuments:` comment, Status-note
      blockquote, Decision-section intro, Decision 1's Node-local-ingest-correction citation,
      AC-004's F-DWSP8-001 qualifying note's Node-local-ingest-correction citation, the Non-Goals/
      AC-017 Gate paragraph's "item (j)" citation, Forward Obligations row (a)'s "item (j)"
      citation, row (d)'s paren-form AND its "item (l)" citation, the Human Gate intro blockquote's
      subsection citation, items 1/2/3's disposition-blockquote citations — "item (k)" ×2, "item
      (j)" — both Architecture Compliance Rules rows, and Task 6's body); the sanctioned historical
      citations left unchanged, consistent with this story's established historical-preservation
      precedent — the `modified:`/Changelog narration of past versions, and the two paragraphs
      newly minted in v2.13 (the AC-001 "Scope note (Ruling 4, v1.10, 2026-07-15)" heading and body,
      and the AC-003 postcondition 1 rewrite's "rulings v1.10 Ruling 2 addendum" citation, both
      point-in-time provenance of a specific ruling's own origin version, not live pointers to the
      currently-binding document) and Forward Obligations row (e) (also newly minted in v2.13, same
      class) all stay pinned at v1.10. `input-hash` recomputed via `compute-input-hash --update`:
      rulings changed on disk again (v1.10→v1.11) — `8bdbc57` → `c8aaf9c`. `acceptance_criteria_count`
      stays 18; points stay 8. **`status` stays `ready`** — Ruling 5 and the new obligation are
      retrospective, implementation-time findings dispatched ahead of the Step-4.5 adversarial loop,
      not new Human Gate items; they do not reopen the story-ready disposition. Frontmatter
      `version` 2.13 → 2.14, new `modified:` entry added.
  - date: 2026-07-15
    version: "2.13"
    change: >
...
> mid-ruling) are ADJUDICATED (`.factory/decisions/S-BL.DISCOVERY-WIRE-rulings.md` v1.11,
> 2026-07-13). This elaboration is `status: ready` — human-gate-approved 2026-07-14 per v2.12;
> all three Human Gate items dispositioned (SEC-DW-07 approved, port `49201` adopted,
> fan-out target resolution resolved to `S-BL.NODE-IDENTIFY-WIRE`). Do not implement from the
> v1.1 stub's "Open Design Obligations"/"Scope"/"Scope Constraints" sections — they are
...
Transcribed from `.factory/decisions/S-BL.DISCOVERY-WIRE-rulings.md` v1.11 (binding — all factual
...
- **`Discovery.ReceiveAdvertisement` is RETIRED (deleted), not preserved — corrected by F-DWSP8-001.**
  ...
  `S-BL.DISCOVERY-WIRE-rulings.md` v1.11, "Node-local ingest correction."
...
Rationale and implementation decisions: `.factory/decisions/S-BL.DISCOVERY-WIRE-map-bounding-ruling.md` v1.2.**
...
- **`RouterIngest.lastSeen`** (`maxLastSeenEntries = 65536`): bounded via **LRU-by-lowest-sequence
  ...
  **Forward-advance** (`seen && sequence > last`) branch: write watermark first (so `k`'s raised sequence makes it an **improbable — not impossible** — LRU victim; benign if evicted, reverting to the accepted one-heartbeat EC-006 cold-start residual), then drain with `for len > maxLastSeenEntries { evictLRULastSeen() }` — this loop is **dead in production** because the cold-start cap ensures `len <= max` before
  ...
  admitted credentials. Delivered green at 545429f. References: ruling Decision 2 + Decision 8; AC-008/AC-009/AC-010; EC-006; SEC-DW-07.
...
2. **Relay-dispatch closure.** On an accept+relay verdict from the ingest path, assembles the
   ...
   half of this is Task 6 (decomposed into 6a/6b/6c/6d, now ungated — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127).**
...
**Scope note (Ruling 4, v1.10, 2026-07-15 — superseded in place by v2.18, 2026-07-20):**
The original v2.13 note stated that `wireDiscoveryListener` "is not called from `runRouter`" and
that this AC's test "currently cannot verify daemon-lifecycle behavior." **Both statements are now
false.** Task 6d (delivered at worktree `feature/S-BL.DISCOVERY-WIRE-FANOUT` @ `af91335`) wired
`wireDiscoveryListener` into `runRouter` via a startup loop iterating
`routerKS.AllSVTNEntries()`. The `discoveryWG` `sync.WaitGroup` and the `onRelay` inline closure
(capturing `router`, `&sendMap`, `relayRateCap`) are live in `runRouter`. A daemon-level join
test — `TestRunRouter_JoinsDiscoveryGroups_ForAdmittedSVTNs` in
`cmd/switchboard/discovery_listener_wire_test.go` — now provides the daemon-lifecycle oracle for
this postcondition. The historical context (Ruling 4 adjudication, the `S-BL.ADMISSION-SYNC-WIRE`
follow-on story for admission-sync wiring, and the Forward Obligation (e) row) is preserved in the
Forward Obligations table. The remaining open residual is **FO(g)**: dynamic
discovery-listener registration for SVTNs admitted AFTER router startup (the
`AllSVTNEntries()` call is a startup snapshot; post-startup SVTN admission does not
automatically wire a new `wireDiscoveryListener` goroutine). FO(g) is tracked in the
Forward Obligations table and is non-blocking.
...
call, per rulings v1.11's Node-local ingest correction. "No call site references it" remains true
post-deletion; no postcondition text change needed, scope clarification only.

**Qualifying note (F-DWIP1-001, v1.11):** `Encode`/`Decode`'s signatures gained an explicit
`nodeAdmissionPubkey []byte` parameter (both route through `routing.DeriveDiscoveryKey`, confirmed
faithful to postcondition 4's already-specified symmetric design) after a Step-4.5 pass-1 fix burst
found the shipped `Encode`/`Decode` had regressed to deriving the discovery HMAC key from cleartext
`SVTNID` alone — the exact anti-pattern DRIFT-W6TBD-001/Ruling 1 already rejected — breaking
sender↔router interop undetected because no prior test exercised a real `Encode`→router-`Ingest`
round-trip. New integration test: `TestDiscovery_EncodeThenRouterIngest_AcceptsRealAdmittedNode`.
`discovery.Config` gained `LocalNodeAdmissionPubkey []byte`; `transmitAdvertisement` fails closed
with `ErrMissingNodeAdmissionPubkey` when empty. Production usability of this fix is itself gated on
a new Forward Obligation — see row (f) and `S-BL.DISCOVERY-WIRE-rulings.md` v1.11 Ruling 5.
...
### AC-017 — [UNGATED — S-BL.NODE-IDENTIFY-WIRE merged PR #127] SVTN-scoped, exclude-originator, best-effort fan-out dispatch (traces to BC-2.03.001 Postcondition 1 delivery-mechanism note)

**BC Anchor:** BC-2.03.001 Postcondition 1 (delivery-mechanism note)

**Gate:** SATISFIED — `S-BL.NODE-IDENTIFY-WIRE` merged at PR #127 (`7fcf0cf`), delivering
`Router.BindInterface`/`LookupInterface`/`UnbindInterface` and the `identityIfaceMap` binding.
The target-resolution mechanism for fan-out dispatch is `Router.InterfacesForSVTN(svtnID [16]byte,
excludeNodeAddr [8]byte) []InterfaceID` per `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md`
v1.0. See Task 6a/6b for the decomposed implementation steps.
...
### AC-018 — [UNGATED — S-BL.NODE-IDENTIFY-WIRE merged PR #127] Relay-dispatch rate cap (traces to BC-2.03.001 Postcondition 5; SEC-DW-09)

**BC Anchor:** BC-2.03.001 Postcondition 5; SEC-DW-09

**Gate:** SATISFIED — same gate as AC-017; `S-BL.NODE-IDENTIFY-WIRE` merged at PR #127 (`7fcf0cf`).
The dispatch mechanism (Task 6b) is now fully specifiable; rate cap is implemented as Task 6c.
...
| (d) | `BC-2.03.002` Postcondition 1's `PENDING-S-BL.DISCOVERY-WIRE` annotation anticipates this story also wiring a `sessions.list` RPC handler exposing `discovery.Enumerate()` over the mgmt wire. **None of the three rulings (v1.11) adjudicate this** — Ruling 1/2/3 scope is exclusively the UDP-multicast advertisement transport (hop-1 ingest + hop-2 relay), not a console-facing enumeration RPC. This is a discovered scope question, not resolved here. | PO / architect | **RESOLVED — 2026-07-14.** Human gate resolved this to a new backlog story, **`S-BL.SESSIONS-LIST-WIRE`** (`stories/S-BL.SESSIONS-LIST-WIRE.md` v1.0, draft, wave backlog). `BC-2.03.002` PC-1's annotation is already re-pointed to that story's ID on disk (`BC-2.03.002.md` v1.5). See rulings v1.11 item (l). | RESOLVED |
| (e) | Router daemon-lifecycle wiring for the discovery multicast listener (AC-001 Postcondition 1): `wireDiscoveryListener` is fully implemented and tested but not called from `runRouter` — the router process has no source of "which SVTN(s) am I serving" (`admission.AdmittedKeySet` has no SVTN-enumeration method; the only production `RegisterKey` caller runs in control-mode, a separate, disconnected OS process from router-mode). Same root cause also blocks `S-BL.NODE-IDENTIFY-WIRE`'s own `AdmitNode` call (verification-only; requires the key already be present in the router's own, always-empty `AdmittedKeySet`). | architect (disposition, Ruling 4) / PO (scoping), then implementer | **RESOLVED — Task 6d delivered** (`wireDiscoveryListener` startup loop wired into `runRouter` iterating `routerKS.AllSVTNEntries()`; `onRelay` inline closure + `discoveryWG` introduced; worktree feature/S-BL.DISCOVERY-WIRE-FANOUT @ `af91335`). Dynamic post-startup SVTN join is the residual, tracked as new FO(g). | RESOLVED |
| (f) | Node-side admission-identity provisioning (F-DWIP1-001 fix burst): no production code path anywhere supplies a running access-node process with its own admission keypair — `internal/config.Config` has no admission-keypair field; `runAccess` constructs only an ephemeral mgmt keypair and its own separate, empty `AdmittedKeySet`. Independently found to be compounded by a second, distinct gap: `internal/discovery.New`/`Discovery.Run` have zero production callers anywhere in the repository — the sender daemon-lifecycle wiring into `runAccess` was never built, independent of identity. Same root cause also blocks `S-BL.NODE-IDENTIFY-WIRE`'s own `ChallengeResponse` signing (needs the node's private key, which nothing provisions either). | architect (disposition, Ruling 5) / PO (scoping), then implementer | **CLOSED — S-BL.NODE-ADMISSION-PROVISIONING (PR #125 @ `ce06f6a`).** Both facets delivered: (1) `loadOrGenerateAdmissionKeypair` generates/loads the access node's stable Ed25519 private key; `discovery.Config.LocalNodeAdmissionPubkey` is populated. (2) `runAccessWithConnector` calls `d.Run(runCtx)` in a goroutine — Discovery daemon lifecycle is now wired into `runAccess`. Confirmed from S-BL.NODE-IDENTIFY-WIRE story Previous Story Intelligence (V11) and `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0 Decision 5. | CLOSED — PR #125 @ ce06f6a |
| (g) | **Dynamic discovery-listener registration for SVTNs admitted AFTER router startup — the PRIMARY bootstrap path in standard cold-start deployment.** `routerKS.AllSVTNEntries()` is a startup snapshot; SVTNs admitted post-startup via `wireAdmissionSyncHandlers` push do NOT get a `wireDiscoveryListener` goroutine in the current process run — no admission-event hook analogous to `nodeConnHook` exists. **Operational materiality:** the standard cold-start deployment sequence is router starts with an empty keyset (logs "starting with empty keyset — awaiting push from control") → control pushes keys post-startup → `AllSVTNEntries()` returns empty at startup → ZERO discovery listeners spawned → the entire hop-2 discovery-relay feature this story delivers is inert until a router restart with a populated snapshot (i.e., until a restart-after-keys-persisted, since post-startup admission-sync pushes do not spawn listeners per this gap). This is not merely an edge case for dynamically-joined SVTNs; it is the primary path. A follow-on story must add an admission-event hook (equivalent to the `nodeConnHook` pattern, but for SVTN admission events) and call `wireDiscoveryListener` for each newly admitted SVTN. This requires new API surface in `wireAdmissionSyncHandlers` or a new `registerSVTNListenerHook` seam — neither is authorized by the current story's File-Change List. | architect (disposition) / PO (scoping), then implementer, in a future story | **OPEN — non-blocking; deferred beyond S-BL.DISCOVERY-WIRE** per `decisions/S-BL.DISCOVERY-WIRE-task6d-wiring-seam-ruling.md` v1.0 Decision 5. | OPEN — deferred |
| (h) | **Full-daemon end-to-end relay fan-out integration test.** The runtime chain `runRouter → wireDiscoveryListener → RouterIngest.Ingest → onRelay → relayRateCap.allow → relayDispatch → peer node's nc.send` is currently covered by unit tests of each link (`TestRelayDispatch_*` 6b, `TestRelayDispatch_RateCap_*` 6c), the `onRelay`-seam branch test (`TestWireDiscoveryListener_InvokesOnRelay_WhenRelayTrue` 6d), the daemon-level join oracle (`TestRunRouter_JoinsDiscoveryGroups_ForAdmittedSVTNs`), and code-inspection of the wiring — but NOT by a single end-to-end integration test that sends an HMAC-authenticated multicast advertisement from one admitted node and observes the `DISCOVERY_RELAY` frame arrive on a second admitted node's live TCP connection (originator excluded). Deferred as too flaky/heavy for a deterministic per-story gate (real multicast + real TCP + NODE_IDENTIFY handshake + timing across goroutines). This formalizes the gap that `TestRunRouter_WithAdmittedSVTN_ShutsDownCleanly`'s own comment discloses (that test is a shutdown-teardown guard, not an e2e relay test — the full relay chain from multicast advertisement to `DISCOVERY_RELAY` frame arrival on a peer's TCP connection is what this FO tracks). | architect (disposition) / PO (scoping), then e2e-tester/test-writer, in a follow-on | **OPEN — non-blocking; deferred beyond S-BL.DISCOVERY-WIRE.** | OPEN — deferred |

**Obligations (a) and (d) are resolved as of 2026-07-14; (f) is CLOSED as of 2026-07-19
(S-BL.NODE-ADMISSION-PROVISIONING PR #125); (e) is FULLY RESOLVED as of 2026-07-20
(Task 6d delivered — `wireDiscoveryListener` startup loop wired into `runRouter`).** (b)/(c)
remain open and non-blocking. (g) and (h) are both newly opened and deferred: (g) dynamic
post-startup SVTN join (per task6d-wiring-seam-ruling.md v1.0 Decision 5); (h) full-daemon e2e
relay fan-out integration test (formalizes the gap disclosed by `TestRunRouter_WithAdmittedSVTN_ShutsDownCleanly`'s
own comment — that test is a shutdown-teardown guard, not an e2e relay test; unit coverage exists for each
relay chain link but no single test drives advertisement → frame arrival across a live TCP connection,
deferred as too flaky/heavy for a per-story gate).
None of (b)/(c)/(d)/(e)/(f) ever blocked TDD implementation of AC-001..AC-016 — (e)/(f) were
retrospective, implementation-time findings (Ruling 4 v1.10, Ruling 5 v1.11) accepted at function
level with qualifying notes on AC-001/AC-004, not `[GATED]` markers. AC-017/AC-018/Task 6 gate is
**SATISFIED** — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127; target-resolution mechanism is
`Router.InterfacesForSVTN` per `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0.
All tasks (1-6d) are delivered.
...
> story-ready gate. See `S-BL.DISCOVERY-WIRE-rulings.md` v1.11, Ruling 3 subsection "Ruling 3(f)
...
> `49201` is the adjudicated discovery UDP port. See rulings v1.11 item (k).
...
> See rulings v1.11 item (j) for full mechanism and rationale, and
...
- **The fan-out target-resolution binding itself (node-identity-to-connection)** — explicitly not
  invented by this story; resolved via `Router.InterfacesForSVTN` per `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0 (Forward Obligation (a) resolved — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127).
...
| EC-008 | An admitted, HMAC-valid, sequence-increasing sender relays advertisements faster than ~1/sec | Registry/discard-map state still updates correctly (AC-010 unaffected); relay dispatch for the excess arrivals is silently suppressed (AC-018). |
...
| Relay-dispatch closure (fan-out + rate cap) | `cmd/switchboard` | effectful-shell | `sendMap`-based socket writes, Task 6b/6c (ungated — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127) |
...
| Router-side ingest MUST perform fixed-offset key-selector extraction before any call to `decodeBody()` (SEC-DW-01, MANDATORY, not optional hardening). | `S-BL.DISCOVERY-WIRE-rulings.md` v1.11, Ruling 1 Implementation Constraint 2 | AC-005 tests; code review. |
...
| The `DISCOVERY_RELAY` relay frame's `HMACTag` MUST be zero — hop-2's trust boundary is the admitted connection, never a per-frame HMAC. | `S-BL.DISCOVERY-WIRE-rulings.md` v1.11, Ruling 3(b) | AC-015 test. |
| Relay dispatch MUST be best-effort, non-blocking (`select`/`default`), never a blocking send that could stall the router on a slow/stuck node. | Ruling 3(d), DRAIN precedent | AC-017 test (S-BL.NODE-IDENTIFY-WIRE merged PR #127; target resolution via `Router.InterfacesForSVTN`). |
...
| `internal/routing/identity.go` | modify | Add `InterfacesForSVTN(svtnID [16]byte, excludeNodeAddr [8]byte) []InterfaceID` — SVTN-scoped connection-enumeration primitive for hop-2 fan-out (Task 6a). |
| `internal/routing/identity_test.go` | extend | Unit tests for `InterfacesForSVTN` (Task 6a). |
| `cmd/switchboard/discovery_wire.go` | create | Router-mode-exclusive multicast listener wiring: bind/join/teardown, calls into `internal/discovery`'s router-side ingest path. Task 6d: gained 6th parameter `onRelay func(discovery.RouterIngestDecision)`; ingest loop threads the decision to caller via `onRelay(decision)` when `decision.Relay == true`; package-doc deferral comment updated to note Task 6d has wired this call and dynamic SVTN join/leave is FO(g). |
| `cmd/switchboard/discovery_wire_test.go` | create | AC-001 (router-mode-exclusive group membership) tests. Task 6d: `callWireDiscoveryListenerRecovered` helper updated to 6-arg form; existing `TestRunRouter_DiscoveryListener_JoinsGroup_RouterModeOnly` call site propagates `nil` as `onRelay` (mechanical update). |
| `cmd/switchboard/discovery_listener_wire_test.go` | create | **NEW (Task 6d)** — four tests: (a) `TestRunRouter_JoinsDiscoveryGroups_ForAdmittedSVTNs` — daemon-level multicast-group-join oracle; this is the RED-gate test for Task 6d and the primary AC-001 daemon-lifecycle verifier; (b) `TestRunRouter_DiscoveryListeners_CleanShutdown` — `discoveryWG` teardown green-guard (verifies clean shutdown with admitted SVTNs after wiring); (c) `TestOnRelayClosureConcurrentAccess` — `relayRateCap` concurrent-access `-race` lock-in; (d) `TestRunRouter_WithAdmittedSVTN_ShutsDownCleanly` — **shutdown-teardown guard** (originally authored as `TestRunRouter_RelayFanOut_EndToEnd` during RED-phase; renamed post-delivery to reflect what it actually asserts — see v2.20 for the rename record): verifies that `runRouter` with an admitted SVTN present starts and shuts down cleanly within a bounded deadline (3s), exercising the `discoveryWG.Wait()` join in the shutdown block on an immediate context cancellation. This is NOT an e2e relay test. The full runtime relay chain (`runRouter` → `RouterIngest` → `onRelay` closure → `relayRateCap.allow` → `relayDispatch` → admitted TCP connection) is NOT exercised end-to-end; per the file's own "JUDGMENT CALL — LIGHTER FLAG-AND-DEFER" and "FULL E2E RELAY COVERAGE GAP" comments, the full e2e relay chain (send HMAC-authenticated multicast datagram → observe `DISCOVERY_RELAY` frame on second admitted node's TCP connection) is deferred to Forward Obligation (h); the relay chain is unit-covered by `TestRelayDispatch_*` (Task 6b, `discovery_relay_wire_test.go`) and `TestRelayDispatch_RateCap_*` (Task 6c, same file) plus code-inspection of the wiring. |
...
| `cmd/switchboard/relay_rate_cap.go` | modify | **SEC-DW-10 map-bounding (map-bounding-ruling.md v1.2 Decision 1):** add `const maxRelayRateCapEntries = 65536`; restructure `allow()` to compute `now := c.now()` once at the top; add amortized prune-by-age sweep `if len(c.last) > maxRelayRateCapEntries/2` deleting all entries with `now.Sub(ts) >= c.interval` before the key lookup. No new imports. |
| `cmd/switchboard/relay_rate_cap_test.go` | create | **SEC-DW-10 RED-first tests (map-bounding-ruling.md v1.2 Decision 4):** new dedicated test file (not an extension of `discovery_relay_wire_test.go`); contains the 3 relayRateCap map-bounding tests + `newFakeClock`/`makeRelayRateKey` helpers: `TestRelayRateCap_MapBounded_AfterStaleEntries` (map size bounded after N distinct keys + clock advance), `TestRelayRateCap_StalePrunedKey_ReAllowed` (pruned key re-encountered returns true), `TestRelayRateCap_ActiveKeys_NotPruned` (fresh-timestamp keys are not pruned). File absent at develop (`7fcf0cf`); first added on feature branch at `5d7cdb1`. All three must FAIL before the Decision 1 implementation is applied. |
| `internal/discovery/discovery_wire.go` | modify | **SEC-DW-10 map-bounding (map-bounding-ruling.md v1.2 Decision 2 + Decision 8):** add `const maxLastSeenEntries = 65536`; add `evictLRULastSeen()` helper method (O(N) scan, selects the key with lowest stored `Sequence` watermark, deletes it; must be called under `ri.mu`). Both-paths bound (Option A, Decision 8): cold-start (`!seen`) branch evicts-before-insert using `for len >= maxLastSeenEntries { evictLRULastSeen() }`; forward-advance (`seen && sequence > last`) branch writes watermark first then drains with `for len > maxLastSeenEntries { evictLRULastSeen() }` (dead in production — cold-start cap keeps `len <= max`; fires only via white-box test pre-inflation). No new imports. Note: this is `internal/discovery/discovery_wire.go` (RouterIngest), NOT `cmd/switchboard/discovery_wire.go`. Delivered green at 545429f. |
| `internal/discovery/discovery_wire_map_bounding_test.go` | create | **SEC-DW-10 RED-first tests (map-bounding-ruling.md v1.2 Decision 4):** new dedicated test file (not an extension of `discovery_wire_test.go`). Contains: `TestRouterIngest_LastSeenMap_BoundedAtCap` (map size bounded after cap insertions, uses white-box pre-loading per Decision 8 rationale), `TestRouterIngest_ReplayRejected_AfterCapEviction` (non-evicted keys still reject replay), `TestRouterIngest_EvictedKey_ColdStartAccepted` (evicted key cold-start accepted — documents ACCEPTED behavior per Decision 8/EC-006, not a regression test), `TestRouterIngest_LastSeen_LRU_EvictsLowestSequence` (LRU selection targets lowest Sequence). All four must FAIL before the Decision 2 implementation is applied. Delivered green at 545429f. |
...
comfortably under half the window. No story split required at 8 points. Task 6 (decomposed 6a/6b/6c/6d) is now ungated — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127 — so all tasks are independently deliverable in a single pass. Pass-3 load remains within the 60% threshold.
...
is `just test-race` green + `just lint` clean before proceeding to the next. **All tasks (1-6d) are
independently deliverable — `S-BL.NODE-IDENTIFY-WIRE` merged PR #127, satisfying the Task 6 fan-out gate;
target resolution is `Router.InterfacesForSVTN` per `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0.**
...
### Task 6a — Routing-layer enumeration primitive (AC-017 prerequisite)

Add `Router.InterfacesForSVTN(svtnID [16]byte, excludeNodeAddr [8]byte) []InterfaceID` to
`internal/routing/identity.go` alongside the already-shipped `BindInterface`, `LookupInterface`,
and `UnbindInterface`. No new file, no new imports.

Red: write unit tests in `internal/routing/identity_test.go`:
- Returns all `InterfaceID`s for `svtnID`, correctly excluding `excludeNodeAddr`.
- Returns non-nil empty slice (`[]InterfaceID{}`) when `svtnID` has no bindings.
...
Green: implement per `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0 Decision 1
— `r.mu.RLock()` for snapshot only; returns freshly-allocated `[]InterfaceID` (never nil; go.md
rule 12); lock released before return. Confirm `go list -deps ./internal/routing` shows no new
`cmd/` imports (ARCH-08 unmodified).

Gate: `just test-race`, `just lint`.

### Task 6b — Hop-2 relay-dispatch closure (AC-017)

Implement the relay-dispatch closure **inline in `runRouter`** (NOT a package var — per
`decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0 Decision 3, the DRAIN-observer
pattern). The closure captures `router` and `sendMap` already in scope in `runRouter`.

Call chain: `router.InterfacesForSVTN(svtnID, originNodeAddr)` → range over `[]InterfaceID` →
`sendMap.Load(ifaceID)` → if `!ok` silent skip → `val.(*nodeConn)` →
`select { case nc.send <- relayFrame: default: }`.

Missing `sendMap` entry (connection closed between snapshot and send) is a silent skip.
`nc.send` is never closed (V6) — the `select`/`default` pattern cannot panic.

Red: write integration tests in `cmd/switchboard/discovery_relay_wire_test.go`:
- Two admitted nodes (A, B) on same SVTN; A sends advertisement; router relays to B; B's send
  channel receives the relay frame.
...
- Missing `sendMap` entry: `IfaceID` in `identityIfaceMap` but absent from `sendMap` → no panic;
  relay continues to remaining targets.
- `go test -race` passes.

Depends on Task 6a. Gate: `just test-race`, `just lint`.

### Task 6c — Relay-dispatch rate cap (AC-018)

Implement `~1/sec` per-`(SVTNID, NodeAddr)` rate cap at the relay-dispatch decision point
(SEC-DW-09). Silent drop on excess-rate arrivals. A `FailureCounter`-shaped visibility counter
is optional and non-gating — never alters the drop decision. Keyed by originating
`(SVTNID, NodeAddr)`.

This is distinct from the hop-1 ingest SEC-DW-03 token bucket (different enforcement point,
different purpose: protects against relay amplification from a legitimately-admitted but
misbehaving sender).

Red: write unit test — two arrivals within 1 second from the same `(svtnID, nodeAddr)`: first
relayed, second silently dropped; no panic, no error return.

Depends on Task 6b. Gate: `just test-race`, `just lint`.

### Task 6d — `wireDiscoveryListener` startup wiring into `runRouter` (Forward Obligation (e) residual)

Add the `wireDiscoveryListener` call loop to `runRouter` startup. Per
`decisions/S-BL.DISCOVERY-WIRE-task6d-wiring-seam-ruling.md` v1.0: construct `ri`
(`discovery.NewRouterIngest`), `relayRateCap` (`newRelayRateCap()`), and `onRelay` inline closure
(captures `router`/`&sendMap`/`relayRateCap`); declare `var discoveryWG sync.WaitGroup`; iterate
`routerKS.AllSVTNEntries()` snapshot; for each `svtnID`, `discoveryWG.Add(1); go
wireDiscoveryListener(ingressCtx, &discoveryWG, svtnID, ri, w, onRelay)`. Add
`discoveryWG.Wait()` to shutdown block between `dataWG.Wait()` and `writerWG.Wait()`.
Wire in the same register-before-serve / WG-tracked pattern as `wireMetricsHandlers`.

Update `discovery_wire.go` package-doc deferral comment: remove "left to a follow-on story once
an SVTN-admission-event source exists" qualifier — condition now met; note dynamic SVTN join/leave
is Forward Obligation (g).

Integration test: verify that `runRouter` with a populated `AdmittedKeySet` (at least one
registered SVTN) joins the corresponding multicast group — extend or reference the existing
`TestRunRouter_DiscoveryListener_JoinsGroup_RouterModeOnly` test.

Depends on Task 6b (listener feeds into the relay-dispatch closure). Gate: `just test-race`,
`just lint`.
...
- **Status at v2.0 elaboration:** `draft`, not `ready` — three items required human/PO disposition
  before wave scheduling (all three later dispositioned 2026-07-14; status promoted to `ready` per
  v2.12 — see Changelog).
...
| 2.17 | 2026-07-20 | **Task 6a-6d delivered — bookkeeping reconciliation pre-Step-4.5 adversary.** Applied `decisions/S-BL.DISCOVERY-WIRE-task6d-wiring-seam-ruling.md` v1.0 (five decisions: `onRelay` seam / backward-compat nil / rate-cap concurrency ownership / `discoveryWG` startup-loop lifecycle / FO(g) dynamic-SVTN deferral). (1) **`inputs:` / `inputDocuments:` updated** — `S-BL.DISCOVERY-WIRE-task6d-wiring-seam-ruling.md` v1.0 added to both lists; `input-hash` recomputed via `compute-input-hash --update`: `c8fe5f2` → `5a4d0da`. (2) **FO(e) status updated** from INFRASTRUCTURE-RESOLVED/Task-6d-residual to **RESOLVED** — `wireDiscoveryListener` startup loop wired into `runRouter` iterating `routerKS.AllSVTNEntries()`; `discoveryWG` WG introduced; `onRelay` inline closure captures `router`, `&sendMap`, `relayRateCap`; worktree feature/S-BL.DISCOVERY-WIRE-FANOUT @ `af91335`. (3) **FO(g) row added** — Dynamic discovery-listener registration for SVTNs admitted AFTER router startup; `AllSVTNEntries()` is a startup snapshot; post-startup SVTN admission via `wireAdmissionSyncHandlers` push does NOT get a `wireDiscoveryListener` goroutine; requires admission-event hook (analogous to `nodeConnHook`); deferred beyond this story per task6d-wiring-seam-ruling.md v1.0 Decision 5; OPEN, non-blocking. (4) **FO summary paragraph updated** — (e) now RESOLVED; (g) newly opened + deferred. (5) **File-Change List updated** — `internal/routing/identity.go` (Task 6a: `InterfacesForSVTN`) and `identity_test.go` added; `discovery_relay_wire.go` now carries `relayDispatch` stateless dispatch alongside assembly (Task 6b, file updated to reflect delivered reality); `relay_rate_cap.go` (NEW — Task 6c, `relayRateCap`); `discovery_wire.go` (6th param `onRelay func(discovery.RouterIngestDecision)`, ingest loop threads decision, Task 6d); `mgmt_wire.go` (gained `internal/discovery` import + `ri`/`relayRateCap`/`onRelay` closure + `discoveryWG` startup loop + `discoveryWG.Wait()` shutdown join — note: ruling's ARCH-08 note said no new imports required, but `internal/discovery` import WAS needed; cmd/switchboard→internal/discovery is a legal DAG edge, no violation); `discovery_listener_wire_test.go` (NEW — Task 6d integration/AC-001 daemon-join tests); `discovery_wire_test.go` (mechanical `callWireDiscoveryListenerRecovered` 6-arg update, nil propagated for `onRelay`). No AC postconditions, test names, or `acceptance_criteria_count` (stays 18) changed. Points stay 8. Status stays `ready`. |
| 2.16 | 2026-07-19 | **AC-017/018 gate-clear sweep — v2.15 follow-on.** Full-body scan for residual `GATED`/`Human Gate item 3`/`Forward Obligation (a)`/`depends_on S-BL.NODE-IDENTIFY-WIRE`-as-gate pins outside the Changelog section. Ten live-body spots swept; all confirmed stale fan-out-gate pins (AC-017/018 now ungated; FO(a) resolved via `Router.InterfacesForSVTN`; fan-out-resolution ruling v1.0). **Fixed spots:** (1) Design Constraint section bullet — `"GATED per the Human Gate section below"` → `"ungated — S-BL.NODE-IDENTIFY-WIRE merged PR #127"`; (2) Non-Goals bullet — `"see Forward Obligation (a) / Human Gate item 3"` → resolved status citing `Router.InterfacesForSVTN` and fan-out-resolution ruling v1.0; (3) EC-008 edge-case row — dropped `"GATED per Human Gate item 3"` qualifier; (4) Architecture Compliance Rules relay-dispatch row — `"GATED — depends_on S-BL.NODE-IDENTIFY-WIRE"` → `"S-BL.NODE-IDENTIFY-WIRE merged PR #127; target resolution via Router.InterfacesForSVTN"`; (5) File-Change List `discovery_relay_wire.go` row — dropped `**GATED**`, updated task references to 6b/6c; (6) File-Change List `discovery_relay_wire_test.go` row — dropped `**GATED** / Human Gate item 3 resolves` framing, updated to `Router.InterfacesForSVTN` per ruling v1.0; (7) File-Change List `mgmt_wire.go` row — `"Task 6, **GATED**"` → `"Task 6b, ungated"`; (8) Token Budget Overall paragraph — `"Task 6's GATED status further reduces..."` rewritten to reflect all tasks independently deliverable; (9) Task Breakdown intro — `"Task 6 is explicitly GATED"` → all tasks ungated per S-BL.NODE-IDENTIFY-WIRE PR #127 merge; (10) Anchors Consumed — hop-2 fan-out row `GATED` → `TO DISCHARGE (ungated)`; fan-out-target-resolution row `NOT DISCHARGEABLE / Forward Obligation (a)` → `TO DISCHARGE / Router.InterfacesForSVTN (FO(a) resolved)`. **Left unchanged (historical/correct):** all hits in the `modified:` YAML block (v2.0–v2.15 historical entries); line-1502 forward-obligations summary `"not [GATED] markers ... gate is SATISFIED"` (correct current-state text, not stale); Human Gate section intro (historical description of what the gate required); Forward Obligations table rows (a)/(d)/(e)/(f) (already show RESOLVED/CLOSED/INFRASTRUCTURE-RESOLVED per v2.12/v2.15). No inputs changed; `input-hash` stays `c8fe5f2` (body-only sweep). `acceptance_criteria_count` stays 18; points stay 8; `status` stays `ready`. |
| 2.15 | 2026-07-19 | **Fan-out-resolution ruling applied** — `decisions/S-BL.DISCOVERY-WIRE-fanout-resolution-ruling.md` v1.0 (committed f7959c44). (1) Task 6 decomposed into 6a (`Router.InterfacesForSVTN` in `internal/routing/identity.go`), 6b (relay-dispatch closure inline in `runRouter`), 6c (AC-018 ~1/sec rate cap, SEC-DW-09), and 6d (`wireDiscoveryListener` wiring call from `runRouter` — FO(e) residual, now unblocked). (2) AC-017/AC-018 `[GATED — depends_on S-BL.NODE-IDENTIFY-WIRE]` tags → `[UNGATED — S-BL.NODE-IDENTIFY-WIRE merged PR #127]`; Gate paragraphs updated to state dependency satisfied and target-resolution mechanism is `Router.InterfacesForSVTN` per ruling v1.0; AC postconditions/test names unchanged. (3) FO (e) → INFRASTRUCTURE-RESOLVED (`wireAdmissionSyncHandlers` in `runRouter` V9; `AllSVTNEntries()`/`ListBySVTN()` V7/V8; Task 6d is the only residual, unblocked); FO (f) → CLOSED (PR #125 @ ce06f6a: `loadOrGenerateAdmissionKeypair` + `d.Run(runCtx)` V11). FO summary updated. Ruling added to `inputs:`/`inputDocuments:`. `input-hash`: c8aaf9c → c8fe5f2. Points stay 8; `acceptance_criteria_count` stays 18. `status` stays `ready`. |
| 2.14 | 2026-07-15 | **Pre-adjudication cascade from `S-BL.DISCOVERY-WIRE-rulings.md` v1.11** — Ruling 5 (the F-DWIP1-001 sender-side key-derivation fix sanctioned as Ruling-1-faithful) plus a new, independently-discovered obligation naming node-side admission-identity provisioning as a THIRD, distinct leg of the identity-distribution cluster, dispatched by team-lead ahead of the Step-4.5 adversarial loop, verified independently against `feature/S-BL.DISCOVERY-WIRE`. **AC-004 gained a SECOND qualifying note (F-DWIP1-001, v1.11), appended after the existing F-DWSP8-001 note:** `Encode`/`Decode`'s signatures gained an explicit `nodeAdmissionPubkey []byte` parameter (both route through `routing.DeriveDiscoveryKey`, confirmed faithful to postcondition 4's already-specified symmetric design) after a Step-4.5 pass-1 fix burst found the shipped `Encode`/`Decode` had regressed to deriving the discovery HMAC key from cleartext `SVTNID` alone — the exact anti-pattern DRIFT-W6TBD-001/Ruling 1 already rejected — breaking sender↔router interop undetected because no prior test exercised a real `Encode`→router-`Ingest` round-trip. New integration test: `TestDiscovery_EncodeThenRouterIngest_AcceptsRealAdmittedNode`. `discovery.Config` gained `LocalNodeAdmissionPubkey []byte`; `transmitAdvertisement` fails closed with `ErrMissingNodeAdmissionPubkey` when empty. Production usability of this fix is itself gated on a new Forward Obligation (row (f)). **Forward Obligations table gained row (f):** node-side admission-identity provisioning — no production code path anywhere supplies a running access-node process with its own admission keypair (`internal/config.Config` has no admission-keypair field; `runAccess` constructs only an ephemeral mgmt keypair and its own separate, empty `AdmittedKeySet`). Independently found to be compounded by a second, distinct gap: `internal/discovery.New`/`Discovery.Run` have zero production callers anywhere in the repository — the sender daemon-lifecycle wiring into `runAccess` was never built, independent of identity. Same root cause also blocks `S-BL.NODE-IDENTIFY-WIRE`'s own `ChallengeResponse` signing (needs the node's private key, which nothing provisions either). New follow-on story recommended: `S-BL.NODE-ADMISSION-PROVISIONING` (working name — not yet created; PO/architect to confirm name + scope), distinct from `S-BL.ADMISSION-SYNC-WIRE` (row (e)) — opposite direction (node learns its OWN identity, vs. router/console learning OTHER nodes' identities). `S-BL.NODE-IDENTIFY-WIRE` must add this story as a SECOND `depends_on` prerequisite alongside `S-BL.ADMISSION-SYNC-WIRE` once both exist. **Forward Obligations summary paragraph replaced:** obligations (a) and (d) resolved as of 2026-07-14; (b)/(c)/(e)/(f) remain open and non-blocking; none of (b)/(c)/(d)/(e)/(f) ever blocked TDD implementation of AC-001..AC-016 — (e)/(f) are retrospective, implementation-time findings (Ruling 4 v1.10, Ruling 5 v1.11) accepted at function level with qualifying notes on AC-001/AC-004, not `[GATED]` markers; AC-017/AC-018/Task 6 gate on `S-BL.NODE-IDENTIFY-WIRE` by name, Tasks 1-5 remain independently deliverable regardless of that story's schedule. **AC-003: no change** — architect confirmed key derivation is AC-004 territory, the Ruling 2 addendum's multicast egress elaboration is untouched by Ruling 5. Mechanical version-pin sweep: every live-prose `rulings v1.10` pin → `v1.11` at 16 spots (the `inputDocuments:` comment, Status-note blockquote, Decision-section intro, Decision 1's Node-local-ingest-correction citation, AC-004's F-DWSP8-001 qualifying note's Node-local-ingest-correction citation, the Non-Goals/AC-017 Gate paragraph's "item (j)" citation, Forward Obligations row (a)'s "item (j)" citation, row (d)'s paren-form AND its "item (l)" citation, the Human Gate intro blockquote's subsection citation, items 1/2/3's disposition-blockquote citations — "item (k)" ×2, "item (j)" — both Architecture Compliance Rules rows, and Task 6's body); the sanctioned historical citations left unchanged, consistent with this story's established historical-preservation precedent — the `modified:`/Changelog narration of past versions, and the two paragraphs newly minted in v2.13 (the AC-001 "Scope note (Ruling 4, v1.10, 2026-07-15)" heading and body, and the AC-003 postcondition 1 rewrite's "rulings v1.10 Ruling 2 addendum" citation, both point-in-time provenance of a specific ruling's own origin version, not live pointers to the currently-binding document) and Forward Obligations row (e) (also newly minted in v2.13, same class) all stay pinned at v1.10. `input-hash` recomputed via `compute-input-hash --update`: rulings changed on disk again (v1.10→v1.11) — `8bdbc57` → `c8aaf9c`. `acceptance_criteria_count` stays 18; points stay 8. **`status` stays `ready`** — Ruling 5 and the new obligation are retrospective, implementation-time findings dispatched ahead of the Step-4.5 adversarial loop, not new Human Gate items; they do not reopen the story-ready disposition. Frontmatter `version` 2.13 → 2.14, new `modified:` entry added. |
