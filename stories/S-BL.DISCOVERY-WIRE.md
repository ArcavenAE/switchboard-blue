...
| `cmd/switchboard/discovery_listener_wire_test.go` | create | **NEW (Task 6d)** — integration and daemon-join tests: full `runRouter` → `wireDiscoveryListener` → `relayDispatch` chain, `discoveryWG` shutdown join, concurrent-`onRelay` `-race` test. |
