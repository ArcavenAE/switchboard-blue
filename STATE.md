...
phase_step: steady-state-access-connector-story-decomposed-v1-1-spec-completeness-resolved-step-4-5-next
product: switchboard
mode: greenfield
...
current_step: "S-BL.ACCESS-CONNECTOR spec pipeline: story-writer DECOMPOSED to v1.1 (13 ACs; re-sync burst propagating product-owner's spec-completeness resolution over the v1.0 decomposition) + spec-completeness pass RESOLVED. Resolved: (1) E-CFG-018/E-CFG-019 registered — BC-2.09.003 v2.2→v2.3 adds PC-16/PC-17 (router_addr host:port validation, svtn_id 32-hex/16-byte validation; EC-026/027/028) as the formal internal/config validation home for these access-mode-only fields, AND error-taxonomy.md v5.2→v5.3 registers both codes in its catalog — corrects the story's original E-CFG-014/E-CFG-015 proposal, which collided with codes already allocated to shipped admission stories (BC-2.09.003 v2.1/v2.2); (2) VP-082 v1.0→v1.1 corrects Test Scenario 3 from a value-inequality negative-space check to a behavioral non-vacuity check — FrameAuthKey is HKDF-deterministic in (svtn_id, node_admission_pubkey) per ARCH-04-admission-security.md's canonical formula and BC-2.05.010's precedent, no handshake-specific entropy; (3) architect placement note v1.0→v1.1 §6 corrected to match (Envelope is byte-identical across a same-node reconnect, not freshly derived); story AC-010 and Residual Open Question 2 (FrameAuthKey entropy) corrected/marked RESOLVED in lockstep. Pre-existing taxonomy-orphan gap surfaced by this pass (E-CFG-007, E-CFG-014-017 defined in BC-2.09.003/BC-2.09.004 but never propagated to error-taxonomy.md's catalog — same class as the earlier E-ADM-022/023/024 orphan pattern) recorded as TAXONOMY-ORPHAN-ECFG in Open Drift Items, deferred to a maintenance sweep, out of this story's scope. BC-INDEX v3.9→v3.10 and VP-INDEX v2.48→v2.49 changelog entries registered (no per-row/count change — neither index carries a per-BC/per-VP version cell). One item remains genuinely OPEN, not resolved this pass (product-owner/architect call, per the story's own Residual Open Questions): whether AC-006 (regular-send composition) and AC-007 (inbound dispatch) warrant dedicated VPs beyond VP-081/VP-082's existing scope. AC count unchanged at 13; points unchanged at 11-13; status stays draft. NEXT: Step-4.5 spec-adversarial convergence (>=3 clean passes) + mandatory fresh-context consistency audit → human spec-approval gate. S-BL.RESYNC-FRAME (Layer 1 protocol machinery, independent/buildable now) and S-BL.CONSOLE-CONNECTOR (design-blocked, RULING-W6TB-C) remain open in parallel. D-chain cite D-446 latest greenfield. trajectory-tail →21→7→4→3"
historical_cycles: []
...
<!--
  ...
  222 lines (wc-l); margin from soft-target = 500 - 200 = 300; margin from actual = 500 - 222 = 278 (D-446(c) dual-margin form). ~22 lines over the 200-line soft target — the residual is structural standing content (14-row Open Drift table, 8-row Wave 6 table, 8-row Phase Progress table, the DISCOVERY-WIRE decision arc), not new burst narrative bloat. Prior burst recorded S-BL.LOOPBACK-FULLSTACK's full delivery (PR #135 squash-merged to `develop` @ `72e6e36d`, 2026-08-30) — see prior commit for detail. This burst registers the S-BL.ACCESS-CONNECTOR spec-completeness resolution (BC-2.09.003 v2.3, VP-082 v1.1, error-taxonomy.md v5.3, BC-INDEX/VP-INDEX changelog entries) and the story-writer v1.1 re-sync decomposition (13 ACs): `phase_step`, `current_step`, `timestamp` updated; one new Open Drift Items row added (TAXONOMY-ORPHAN-ECFG, pre-existing catalog-propagation gap surfaced by the completeness pass). Line count 221→222 (+1 row).
  Hard cap: 500 lines.
-->
...
| TAXONOMY-ORPHAN-ECFG | MED | E-CFG-007 and E-CFG-014–017 are defined in BC-2.09.003 (and referenced in BC-2.09.004) Error Codes tables but were never propagated to the canonical error-taxonomy.md catalog — same class as the E-ADM-022/023/024 orphan pattern. Surfaced by the S-BL.ACCESS-CONNECTOR product-owner completeness pass (which nearly caused an E-CFG-014/015 collision). | product-owner | pre-existing, deferred to a spec-hygiene / maintenance sweep — out of S-BL.ACCESS-CONNECTOR scope; target: maintenance (unscheduled) |
