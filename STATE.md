...
awaiting: "S-BL.PE-RECEIVE-LOOP spec adversarial pass 9 (story v1.8 + note v1.7, streak 0/3)"
historical_cycles: []
...
| 2026-07-09 | **S-BL.PE-RECEIVE-LOOP spec pass 4 REMEDIATED — 2 findings (2H) both closed; note v1.4 (FrameFn return-value contract + SetFrameCallback ordering); story v1.4; pass 5 next (streak 0/3)** | completed | F-SP4-001 (HIGH [spec-gap]): FrameFn return-value contract — errcheck-forced decision whose idiomatic exit-on-error form defeats pin test; discard-and-continue (`_ = frameFn(hdr, raw)`) mandated; FORBIDDEN exit-on-error form documented; double-count rationale; netingress.ServeConn precedent. F-SP4-002 (HIGH [spec-gap]): SetFrameCallback ordering contract — MUST be called before Start(); goroutine-creation happens-before covers visibility; construct→SetFrameCallback→Start binding; nil-guard defense-in-depth; post-Start mutation forbidden. v1.3 remediations cleared under direct attack (byte-contract round-trip, pin test valid, blast-radius 8 complete). Note v1.3→v1.4, story v1.3→v1.4. sprint-state v2.08→v2.09. STORY-INDEX v4.43→v4.44. |
