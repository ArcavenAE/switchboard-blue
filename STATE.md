...
awaiting: "S-BL.PE-RECEIVE-LOOP spec adversarial pass 10 (story v1.9 + note v1.7, streak 0/3)"
historical_cycles: []
...
| 2026-07-09 | **S-BL.PE-RECEIVE-LOOP spec pass 6 REMEDIATED — 2 HIGH + 1 MED + 1 LOW; note v1.6; story v1.6; index v4.46; pass 7 next (streak 0/3)** | completed | F-SP6-001 (HIGH [spec-defect]): v1.5 read-error resync mechanism nonexistent — maintainConn never reads conn; fixed via binding conn.Close() on read-error exit. F-SP6-002 (HIGH [spec-gap]): Handle-interface blast radius — RULED Option A: SetFrameCallback concrete-only, fakeConnectorHandle unaffected. F-SP6-003 (MED [spec-defect]): AC-001 PC-3/AC-004 Mode() unassertable under runRouter harness — observable substitutes: peWriteFixture.accepted / mode=PE line. F-SP6-004 (LOW [doc-drift]): blast radius 8→10 — frame_test.go :501/:540 stale comments. P1-3 transition-ownership trace CLEAN. Streak stays 0/3. Note v1.5→v1.6, story v1.5→v1.6. sprint-state v2.10→v2.11. STORY-INDEX v4.45→v4.46. |
| 2026-07-09 | **S-BL.PE-RECEIVE-LOOP spec pass 5 REMEDIATED — 1 HIGH + 2 LOW; note v1.5 (READ-error disposition); story v1.5; index v4.45; pass 6 next (streak 0/3)** | completed | F-SP5-001 (HIGH [spec-gap]): READ-error disposition unspecified — v1.4 callback rule (non-nil frameFn return MUST NOT terminate loop) created a wrong-direction trap at the read site: idiomatic `if _, err := frame.ReadOuterFrame(...); err != nil { return }` exits on EOF/disconnect, which is correct, but the v1.4 blanket "MUST NOT terminate" rule contradicted this. Note v1.5 adds the READ-error contract: `io.EOF`/`io.ErrUnexpectedEOF`/`net.Error` on `ReadOuterFrame` MUST terminate the loop (upstream disconnected); non-nil `frameFn` return MUST NOT (drop-and-continue). New pin test `TestConnector_ReceiveLoop_ExitsOnReadError`. F-SP5-OBS-1 (LOW [spec-divergence]): bounded-read note vs unbounded loop — accepted with rationale (receive goroutine exits on read error; no separate bounded-read contract needed). F-SP5-OBS-2 (LOW [spec-completeness]): connector_test.go fixture pattern clarification added. Note v1.4→v1.5, story v1.4→v1.5. sprint-state v2.09→v2.10. STORY-INDEX v4.44→v4.45. |
