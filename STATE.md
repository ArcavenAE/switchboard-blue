...
| DRIFT-HS006-ROUTER-DAEMON-STUB | MEDIUM | Router daemon subcommand `./bin/switchboard router` prints `runRouter: not implemented` and exits 1. Steps 9 & 10 of HS-006 cannot be exercised through operator surface. Deferred to follow-on router-daemon-runtime story. | orchestrator | open |
...
| DRIFT-HS006-DRAIN-TIMEOUT-FORCED-EXIT-UNEVIDENCED | LOW | Drain-timeout forced-exit-with-log clause not observable through public API with router daemon stubbed. Re-evaluate when DRIFT-HS006-ROUTER-DAEMON-STUB is closed. | orchestrator | open |
