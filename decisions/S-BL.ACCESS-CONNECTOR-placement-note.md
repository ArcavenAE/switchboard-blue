...
- **What IS discarded and rebuilt**: exactly the `net.Conn` and the `outerassembler.Envelope` derived from the OLD admission handshake (`SrcAddr`/`DstAddr`/`SVTNID`/`FrameAuthKey` — `SVTNID` is actually invariant across reconnects since it's config-sourced, §1, not handshake-derived, but `SrcAddr`/`DstAddr`/`FrameAuthKey` are freshly derived by each new `admit()` call, §4.2). VP-082's Test Scenario 3 (negative-space check: old envelope ≠ new envelope) is satisfiable directly from this — the two envelopes differ in practice because `FrameAuthKey`'s HKDF derivation is fresh per handshake even when `SrcAddr`/`SVTNID` land on the same value both times (implementer should confirm the derivation includes handshake-specific entropy, e.g. the challenge nonce, so this holds even under a byte-identical keypair/SVTN pairing — flagged as an implementation-time check, not a design gap).
