...
| DRIFT-001b | LOW | consistency-validator (H-003/M-002/L-003) | BC-2.06.001 PC-5 console session-list half: the console session-list view surfacing of the quality indicator remains deferred. BC-2.06.001 PC-5 second clause ("console's session list view") is not yet owned by any wave story. | S-7.03 (console session-list surfacing — Wave 6) | open |
| DRIFT-002 | LOW | S-5.01 adversarial convergence Pass 1 (F-001) | BC-2.06.002 PC-3 gap-event observability surface: the missCount counter increment is the internal path-metric record (disposition 1, BC-2.06.002 v1.2). The operator-visible export of that counter (e.g. via `sbctl sessions status` or console session-list) is deferred. Re-anchored to S-7.03 (console wiring, Wave 6) per Task 5 reconverge ruling. | S-7.03 (console wiring — Wave 6) | open |
