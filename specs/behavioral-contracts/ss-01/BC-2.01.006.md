...
3. A cryptographic hash function is available (implementation: SHA-256 truncated to 8 bytes or equivalent collision-resistant function).
...
| EC-004 | Attempt to spoof a node address (forged source address in outer header) | HMAC verification fails at router (E-ADM-002) — the HMAC is keyed to the legitimate node's admission key, which the forger does not have. |

## Canonical Test Vectors

| Input | Expected Output | Category |
|-------|----------------|----------|
| svtn_id=0x0102...(16B), pubkey=ed25519 test key | address = SHA256(svtn_id \|\| pubkey)[0:8] (deterministic) | happy-path |
...
|--------|----------|-------------|
| VP-TBD | address(svtn_id, pk) is deterministic: same inputs → same output | unit |
| VP-TBD | address(svtn_id1, pk) != address(svtn_id2, pk) for svtn_id1 != svtn_id2 | proptest |
| VP-TBD | address does not contain or leak the private key | code-audit |
