...
| BC-2.04.009 | Access-node connection establishment / dial + genuine NODE_IDENTIFY admission + live ARQ send-path wiring | session-access | CAP-013 | P2 | PE | active | ss-04/BC-2.04.009.md |
...
| BC-2.06.004 | On-demand single-target reachability probe via sbctl paths ping | quality-observability | CAP-029 | P2 | E | active | ss-06/BC-2.06.004.md |
...
| session-access | CAP-013–016 | 9 | 7 | 2 | 0 |
...
| **Total** | **CAP-001–029 + CAP-020a, CAP-020b** | **54** | **45** | **9** | **0** |
...
| CAP-013 | BC-2.04.001, BC-2.04.002, BC-2.04.007, BC-2.04.009 | covered |
...
| CAP-029 | BC-2.06.004 | covered |

## Changelog

| Version | Date | Change |
|---------|------|--------|
| 3.10 | 2026-08-31 | S-BL.ACCESS-CONNECTOR spec-completeness resolution (product-owner, item 4 of 4): BC-2.09.003 v2.2→v2.3 — PC-16 (`router_addr`: required for access mode, host:port validated via the existing `validateHostPort` helper, E-CFG-018) and PC-17 (`svtn_id`: required for access mode, 32-lowercase-hex-char decoding to 16 bytes via `encoding/hex.DecodeString`, no I/O in `Validate()`, E-CFG-019) added — the formal `internal/config` validation home for the access-mode-only config fields BC-2.04.009 Precondition 2 sources but declines to itself validate (resolves placement-note §11 item 3). EC-026 through EC-028 added; Canonical Test Vectors and test-as-evidence rows added (no formal VP, mirroring PC-12–PC-15's own treatment). Same pattern as v2.1/v2.2's PC-12–PC-15 additions: no row/coverage change — CAP-028, deployment-operations, Status `implemented (S-6.01 / PR #28)` all unaffected; BC-INDEX carries no per-BC version column to update. BC count unchanged at 54. This resolution surfaced a pre-existing, out-of-scope taxonomy-orphan gap (E-CFG-007, E-CFG-014–017 defined in this BC's own Error Codes table but never propagated to `error-taxonomy.md`'s catalog) — tracked as `TAXONOMY-ORPHAN-ECFG` in `.factory/STATE.md` Open Drift Items, not remediated here. |
| 3.9 | 2026-08-31 | S-BL.ACCESS-CONNECTOR spec-authoring burst: **1 new BC** — BC-2.04.009 v1.0 commissioned (access-node dials its router, completes genuine client-side NODE_IDENTIFY admission per BC-2.01.009, and wires the live ARQ send-path; session-access / CAP-013 / `internal/accessdial`; P2/PE; traces to backlog-stub story S-BL.ACCESS-CONNECTOR). **2 amended BCs** — BC-2.04.001 v1.1→v1.2 and BC-2.04.003 v1.3→v1.4 (mechanical citation only, per `S-BL.DATAPLANE-CONNECTOR-scoping-note.md` §5 item 1: Precondition/Description and Related-BCs cross-referenced to the new BC-2.04.009 as the authority for the connection-establishment mechanism each already presupposed; no behavioral content changed, both remain transport-agnostic). BC count 53→54; session-access count 8→9, Scope-PE count 8→9 (Scope-E count unchanged at 7 for the subsystem / 45 overall). CAP-013 coverage row updated. |
