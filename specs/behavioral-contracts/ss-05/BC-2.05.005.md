...
modified:
  ...
  - '2026-07-06: v1.9 — wording alignment: EC-011 and Window-semantics clause updated to clarify that dead-key eviction (Step 2 in RecordHMACFailure) IS the drain-only re-arm mechanism; the separate Step-3 re-arm block that previously appeared in the implementation was dead code and has been deleted (DRIFT-P6-ADM-STEP3-DEADCODE, PR #108). No behavioral change — observable contract identical (SW305-M4 guard effective via Step 2 throughout; mutation proof in secscan-lane-report.md §Mutation mutation-15).'
deprecated: null
deprecated_by: null
replacement: null
retired: null
...
3. **Per-source HMAC failure rate alert:** When `RouteFrame` returns `ErrHMACVerificationFailed` for a frame from `src_addr`, it MUST call `admission.RecordHMACFailure(srcAddr string)` on the router's failure recorder before returning. The `FailureCounter` maintains a per-`src_addr` sliding-window counter over a 60-second window. When the count for a `src_addr` reaches or exceeds **5** within any trailing 60-second window, the `FailureCounter` emits a structured log event via the injected `Logger` interface (`Log(msg string)`) with code **E-ADM-017** ("E-ADM-017 HMAC failure rate alert: ≥`<threshold>` failures in `<window_seconds>`s from src `<src_addr>`"). The message embeds the code literal "E-ADM-017" for operator grep-ability. Severity is **not** encoded as a logger level (the `Logger` seam is level-less); operator severity is defined by the error taxonomy as `degraded` (daemon continues). Operators may route the message to an error-level sink by checking for the "E-ADM-017" prefix in their log pipeline.
   ...
   **Admission-layer API contract** (the seam the implementer builds — no Go code, but the contract is precise):
   ...
   - Window semantics: **sliding** (not fixed-bucket) — at each `RecordHMACFailure` call, timestamps older than `now - windowDuration` are trimmed; the count of remaining entries (after trim, and before append if append is permitted — see per-source slice bound above) determines threshold comparison. After trim, if **`len(keep) == 0`** (all pre-fire entries have aged out), dead-key eviction deletes both `counts[srcAddr]` and `firedAt[srcAddr]` — this is the drain-only re-arm mechanism; the normal append step then proceeds and threshold counting resumes. The `keep[0].After(firedAt[srcAddr])` disjunct that appeared in v1.5 is dead under the append-skip policy — because no post-fire timestamp is ever appended, `keep[0]` can never be newer than `firedAt[srcAddr]` while `firedAt` is set — and is removed from the authoritative spec. Note: a previously present Step-3 block that checked `!lastFire.IsZero() && len(keep)==0` was also dead code (proven by mutation-15 in secscan-lane-report.md) and has been removed (v1.9). Dead-key eviction (Step 2) subsumes all re-arm logic completely.
