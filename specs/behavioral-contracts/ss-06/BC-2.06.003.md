...
1. **[CANONICAL]** `sbctl paths list` returns a list of all active paths for the node, each with the following fields:
   ...
   - `router_addr` — remote router address (host:port). Populated from `PathSnapshot.RouterAddr`, which is set at `PathTracker` construction time via `NewPathTrackerWithAddr`. A `PathTracker` constructed via `NewPathTracker` (addr-less) yields `router_addr: ""`. End-to-end observability (non-empty `router_addr` from a live daemon) SATISFIED as of v1.16: `S-BL.PATH-TRACKER-WIRING` shipped (PR #99, c098827) — production trackers register via routing's `ForwardingEntryHook` → `pathTrackerSource.Register(svtnID, nodeAddr)` (idempotent across auth-key rotations; RWMutex-arbitrated). DRIFT-SW504-ROUTER_ADDR-PLACEHOLDER closed by `S-BL.ROUTER-ADDR`.
   ...
   - `status` — path health classification: `active` | `degraded` (RTT > 200ms sustained) | `failed` (liveness failure: previously-alive path no longer responding). The `failed` value is NORMATIVE as of v1.16 — `S-BL.PATH-FAILED-STATUS` shipped (PR #99, c098827); the Wave-6 Ruling-4 reservation is lifted.

   **Status derivation (Ruling-9, 2026-07-01; extended v1.16 per S-BL.PATH-FAILED-STATUS):** precedence Failed > Degraded > Active. `status == "failed"` iff `PathSnapshot.Failed == true` (liveness signal set only when `!firstProbe` — a path that was up and went down; cleared by `resetRTT` on reactivation). Otherwise `status == "active"` iff `PathSnapshot.Active == true AND PathSnapshot.Degraded == false`; all remaining combinations map to `"degraded"`. Ruling-9 fall-through preserved: a never-alive path with `Active == false` reports `"degraded"`, NOT `"failed"` — `"failed"` reserves the "was up, went down" operator meaning.
...
3. **[ALIAS]** `sbctl router status --target <router>` is a convenience alias for `sbctl paths list`. It produces an equivalent per-path listing (same JSON schema as PC-1) with an additional `quality` column (green/yellow/red quality indicator derived from the status + rtt_p99_ms fields). Both commands route through the same underlying query path in `internal/metrics`; there are no divergent code paths. The `--target <router>` flag overrides the default daemon address, equivalent to `sbctl --target <router> paths list`. The alias exists to match the command surface introduced by S-5.02 (F-P8-002 ruling).
   ...
   **Failed-status extension (v1.16, S-BL.PATH-FAILED-STATUS shipped PR #99):** the S502-DEFER-3 precedence rule now covers the full status vocabulary: `status: "failed"` + `SampleCount < 10` → `quality: "pending"` (same orthogonality principle). `"failed"` NEVER appears as a quality value — the quality enum remains `{green, yellow, red, pending}`. Locked by test `row_e_failed_and_pending`.
...
| EC-007 | `sbctl router status --target <router>` on a path with ≥3 consecutive missed keep-alives (liveness failure) AND fewer than 10 RTT samples collected | **v1.16 (Ruling-4 reservation LIFTED — S-BL.PATH-FAILED-STATUS shipped PR #99):** `status: "failed"` IS emitted (liveness failure on a previously-alive path). Pending-precedence applies: `SampleCount < 10` → `rtt_p99_ms: "pending"` AND `quality: "pending"`. The `quality` field MUST NOT be `"failed"` — not a valid quality enum value; `status` and `quality` remain orthogonal. Locked by test `row_e_failed_and_pending`. |
...
|---------|------|--------|--------|
| 1.16 | 2026-07-05 | orchestrator (coordinator burst, S-BL.PATH-FAILED-STATUS + S-BL.PATH-TRACKER-WIRING shipped PR #99 c098827) | Ruling-4 reservation LIFTED: PC-1 status enum widened `{active, degraded}` → `{active, degraded, failed}` (NORMATIVE). Status derivation extended: precedence Failed > Degraded > Active; `failed` iff `PathSnapshot.Failed == true` (set only when `!firstProbe`; cleared by `resetRTT`); Ruling-9 fall-through preserved (never-alive path reports `degraded`, not `failed`). PC-3 Wave-7 forward-looking paragraph replaced with normative failed-status extension of S502-DEFER-3 (failed + SampleCount<10 → quality "pending"; `failed` never a quality value; locked by `row_e_failed_and_pending`). EC-007 rewritten from RESERVED to normative-emit. PC-1 `router_addr` S-BL.PATH-TRACKER-WIRING deferral note now satisfied (production wiring landed same PR — hook-callback registration via ForwardingEntryHook). |
