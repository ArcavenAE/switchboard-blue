...
modified:
  - 2026-07-12T00:00:00 # v2.13 — S-BL.LOOPBACK-FULLSTACK (DRAFT, pending story delivery): §6.5 pos-23 (internal/testenv) import set {admission, drain, frame, outerassembler, session, upstreamdial} → {admission, arq, drain, frame, halfchannel, multipath, outerassembler, paths, session, upstreamdial}. Adds halfchannel (pos 5), arq (pos 9), multipath (pos 14), and the transitively-required paths (pos 11) — multipath.NewMultipath/Send take []paths.RankedPath, so any importer constructing a *Multipath must import internal/paths directly (Go does not permit referencing a transitive package's exported types without a direct import). All four additions are strictly below position 23 — lawful forward edges, no cycle. Purity rationale: testenv is already effectful test infrastructure (goroutines, tickers, net.Listen — see existing StartRouter/newEnv); the pure-core boundary of halfchannel/arq/multipath themselves is UNCHANGED by this edge — testenv becomes an effectful DRIVER of their Tick()/OnAck()/Send() entry points, the same relationship production code (internal/tmux, internal/arqsend, cmd/switchboard/mgmt_wire.go) already has with these packages. No forbidden edge per §6.2 (halfchannel/arq/multipath do not gain any import; testenv is a leaf, imported by nothing outside _test files). STATUS: DRAFT / PROSPECTIVE — pending S-BL.LOOPBACK-FULLSTACK story delivery; import set to be machine-verified (go list) at merge per the testenv v2.5/v2.8/v2.11 precedent. See `.factory/decisions/S-BL.LOOPBACK-FULLSTACK-placement-note.md` for the wiring design and purity/rationale detail.
  - 2026-07-11T00:00:00 # v2.12 — S-7.04-FU-DRAIN-WIRE: §6.5 pos-18 (internal/netingress) import set {frame} → {frame, routing}; new edge internal/netingress (pos 18) → internal/routing (pos 17) is a forward edge (imports position < own position), LAWFUL, no cycle. Refs: S-7.04-FU-DRAIN-WIRE + code branch feature/S-7.04-FU-DRAIN-WIRE@e7614d7 — code lands on the feature branch, same-delivery-burst with this spec-doc update.
...
| 18 | `internal/netingress` | {frame, routing} | boundary | Wave 7 (S-BL.NI, PR #94, b8ed015); routing edge added S-7.04-FU-DRAIN-WIRE |
...
| 23 | `internal/testenv` | {admission, arq, drain, frame, halfchannel, multipath, outerassembler, paths, session, upstreamdial} (**PROSPECTIVE** — arq pos 9, halfchannel pos 5, multipath pos 14, and the transitively-required paths pos 11 added per S-BL.LOOPBACK-FULLSTACK v2.13, DRAFT pending story delivery; all four below 23, no back-edges; final machine-verification at merge. Prior state (updated at story branch 51ecd44 pre-merge — S-7.04-FU-PE-CONNECTOR added outerassembler + upstreamdial edges) was machine-verified at 62e38d3.) | test helper (composition root — importable by _test files only) | steady-state (S-BL.TESTENV, PR #110, 62e38d3); loopback extension PROSPECTIVE (S-BL.LOOPBACK-FULLSTACK) |
...
Position 23 (`internal/testenv`)'s {admission, drain, frame, outerassembler, session,
upstreamdial} subset remains machine-verified at 62e38d3 / 51ecd44 as above; the
{arq, halfchannel, multipath, paths} addition (v2.13) is PROSPECTIVE pre-code
registration for `S-BL.LOOPBACK-FULLSTACK` (not yet machine-verified — no code has
landed) and must be re-verified via `go list` at that story's merge, per the same
protocol §6.4 requires of every pre-code registration.
...
| Version | Date | Change |
|---------|------|--------|
| 2.13 | 2026-07-12 | S-BL.LOOPBACK-FULLSTACK (DRAFT, pending story delivery): §6.5 pos-23 (`internal/testenv`) import set `{admission, drain, frame, outerassembler, session, upstreamdial}` → `{admission, arq, drain, frame, halfchannel, multipath, outerassembler, paths, session, upstreamdial}`. Adds `halfchannel` (pos 5), `arq` (pos 9), `multipath` (pos 14), and the transitively-required `paths` (pos 11) — `multipath.NewMultipath`/`Send` take `[]paths.RankedPath`, so any importer constructing a `*Multipath` must import `internal/paths` directly. All four additions are strictly below position 23 — lawful forward edges, no cycle. Purity rationale: testenv is already effectful test infrastructure; the pure-core boundary of halfchannel/arq/multipath is unchanged — testenv becomes an effectful driver of their `Tick()`/`OnAck()`/`Send()` entry points, mirroring the relationship production code (`internal/tmux`, `internal/arqsend`, `cmd/switchboard/mgmt_wire.go`) already has with these packages. STATUS: DRAFT/PROSPECTIVE — pending `S-BL.LOOPBACK-FULLSTACK` story delivery; import set to be machine-verified at merge per the testenv v2.5/v2.8/v2.11 precedent. Design detail: `.factory/decisions/S-BL.LOOPBACK-FULLSTACK-placement-note.md`. Also reorders this table to newest-first (previously oldest-first at the top, scrambled from v2.4 onward) to satisfy changelog-monotonicity validation; no content changed in pre-existing rows. |
| 2.12 | 2026-07-11 | S-7.04-FU-DRAIN-WIRE: §6.5 pos-18 (`internal/netingress`) import set updated `{frame}` → `{frame, routing}`. New edge `internal/netingress` (pos 18) → `internal/routing` (pos 17) is a forward edge (position 18 importing position 17, which is < 18) — LAWFUL, no cycle. Refs: S-7.04-FU-DRAIN-WIRE + code branch `feature/S-7.04-FU-DRAIN-WIRE@e7614d7` — code lands on the feature branch, structurally a different branch from this spec-doc commit; the wire-format spec-pair obligation is satisfied as same-delivery-burst rather than same-commit. |
