...
| E-ADM-020 | ADM | broken | 1 | `"bootstrap-key-revoke-forbidden: cannot revoke the last bootstrap key in SVTN <svtn_id>"` | BC-2.05.004 EC-007 (bootstrap-key revoke protection); emitted by the `admin.key.revoke` handler in `cmd/switchboard/admin_handlers.go` when `svtnmgmt.ErrBootstrapKeyRevokeForbidden` is returned — i.e., the caller attempts to revoke the last remaining control key (bootstrap key) in a SVTN, which would leave the SVTN in an unrecoverable state with no management authority. Distinct from E-ADM-011 (hierarchy violation: lower-tier role cannot revoke higher-tier key), E-ADM-018 (control-to-control revocation requires explicit confirmation), and E-ADM-009 (caller lacks control authority). `<svtn_id>` is the target SVTN identifier. The Go sentinel `svtnmgmt.ErrBootstrapKeyRevokeForbidden` maps exclusively to this code. |
