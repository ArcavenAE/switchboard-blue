...
4. Console C (read-only key) attaches. It receives the downstream stream. Console C sends an upstream keystroke. The access node rejects it (E-SES-003). Consoles A and B are unaffected — they still receive frames.
...
# Step 4: C attaches; C sends upstream; assert E-SES-003; assert A and B still receive
...
- **Error quality** (0.1): E-SES-003 returned for C's upstream keystroke.
