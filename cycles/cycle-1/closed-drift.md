...
| R18-EQUIV-L256-SWEEP | non-gating, procedural note | CLOSED 2026-08-29 (extracted from STATE.md at the §1.7 audit-2 compaction pass) — R18's F-R18-LENSC-01 de-anchoring touched 2 `L256` citations inside the S-BL.LOOPBACK-FULLSTACK placement-note's v1.5-R3-narrative section (part of the note's 4 total note-side sites) — a MANDATED citation-correctness sweep the adversary named exact sites for (v1.12 Finding #1 multi-section precedent), NOT a substantive historical rewrite: only naked line numbers replaced with mechanism anchors; dated changelog rows and the v1.5 narrative's own conclusions unchanged. | orchestrator | CLOSED (procedural) |
| R19-M2-REJECTED-OPTION-LOOSE-BOUND | documented non-defect | CLOSED 2026-08-29 (extracted from STATE.md at the §1.7 audit-2 compaction pass) — S-BL.LOOPBACK-FULLSTACK R19 Lens B observation: the §M2 rejected-option prose describing the discarded construction-time-start alternative ("more than 64 ... intervals later") is a loose approximation of the exact `E>=64` boundary, in DISCARDED-option prose. Examined and explicitly ruled immaterial/non-defect by Lens B. | orchestrator | CLOSED (non-gating) |
| LEDGER-PARITY-CONVENTION-DECLARED | resolved-in-cycle, root-cause fix | CLOSED 2026-08-29 (extracted from STATE.md at the §1.7 audit-2 compaction pass) — S-BL.LOOPBACK-FULLSTACK R20 remediation declared a ledger-parity convention (L57 provenance / L72 status-note mirror the formal changelog's finding-set 1:1 per version, v1.13 forward; pre-v1.13 grandfathered), closing the recurring ledger-drift class (R19-LENSA-01 → R20-LENSA-01/C-01, R20-LENSA-02/C-02) and the Lens C `[process-gap]` observation naming it. Future passes check ledger parity against this convention (still governing as of R32/audit-2). | architect / story-writer | CLOSED (convention now governs) |
| O-R20-LENSB-01 | documented non-defect | CLOSED 2026-08-29 (extracted from STATE.md at the §1.7 audit-2 compaction pass) — S-BL.LOOPBACK-FULLSTACK R20 Lens B observation: `OnAck` placement differs structurally between note Q4 (ticker-body, after Send) and story Q4/AC-005 (deliverUpstream-adjacent); both independently verified idempotent (`OnAck` count==1 via `arq.go:220`'s `chanSeq-nextExpected==0` guard). Note's ticker-body form governs. Not a correctness defect. | orchestrator | CLOSED (non-gating) |
| O-R20-LENSB-02 | NITPICK | CLOSED 2026-08-29 (extracted from STATE.md at the §1.7 audit-2 compaction pass) — S-BL.LOOPBACK-FULLSTACK R20 Lens B observation: `loopbackSink.SendInput` code sketch omits the `downstreamHCMu` acquisition comment that H2/Task 6 bind correctly elsewhere in the story. Cosmetic; an implementer following Task 6 gets it right regardless. | orchestrator | CLOSED (non-gating) |
