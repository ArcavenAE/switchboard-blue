...
gate). SPEC-2 and SPEC-4 were expected-fail on issue #89 (error-path
double-print broke whole-stream JSON parse) — found by this
harness's first run, which was the validation the pattern needed.
The single-print contract was landed and the annotations were
removed; both entries are now required-pass.
